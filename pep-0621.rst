...
The push for determinism, pared with static metadata, also helps with
disambiguation for when metadata isn't specified. This PEP asks people
to specify which fields are meant to be dynamically specified, thus
if a field is unspecified and not specified as dynamic, then it is
unambiguous that the field was left out purposefully and an acceptable
default value should be assumed.
...
3. If both ``email`` and ``name`` are provided, the value goes in
   ...
   format ``{name} <{email}>``.
4. Multiple values should be separated by ``, ``.
...
for fields. For instace, build back-ends could take the version
