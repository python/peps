...
:pep:`518`. The lack of a ``[project]`` table implicitly means all
fields will be dynamically provided by the build tool.
...
supports a ``Maintainer`` field which going unused by this PEP. The
authors of this PEP felt this field was ambiguous and never clearly
defined. Other ecosystems have selected "author" as the term so use.
