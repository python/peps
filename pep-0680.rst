...
comparison, ``requests`` has about 28000 reverse dependencies.
...
The release of TOML 1.0 in January 2021 indicates stability. Empirically, TOML
has proven to be a stable format even prior to the release of TOML 1.0. From the
...
For more details, see the `Alternative names for module`_ section.
...
* ``toml`` is a very widely used library. However, it is not actively
  maintained, does not support TOML 1.0 and has several known bugs. Its API is
...
#. This PEP currently proposes not to include a write API. That is, there will
   ...
   Discussed at `Including an API for writing TOML`_.
   ...
   that is very different from a compatible API).
...
#. Different first argument of ``toml.load``
   ...
   This is pretty different from the first argument proposed in this PEP:
   ``SupportsRead[bytes]``.

   Recapping the reasons for this, previously mentioned at
   `Types accepted by the first argument of tomllib.load`_:
   ...
   * Using ``SupportsRead[bytes]`` allows us to ensure UTF-8 is the encoding used,
     and avoid incorrectly parsing single carriage returns as valid TOML due to
