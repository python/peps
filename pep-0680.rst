...
:pep:`517`, :pep:`518` and :pep:`621`, and most Python build tools
have to vendor a TOML parsing package. Including TOML support in the standard
library helps avoid bootstrapping problems for Python packaging tools.

Further, many Python tools are increasingly configurable via TOML, such as
...
are converted to instances of the Python ``float`` type.
...
stable format even prior to the release of TOML 1.0. From the
...
major breaking changes, we should preserve support for TOML 1.0.
...
* ``tomlkit`` is well established, actively maintained and supports TOML 1.0.
...
* ``toml`` is a very widely used library. However, it is not actively
  maintained, does not support TOML 1.0 and has a number of known bugs. Its API
  is more complex than that of ``tomli``. It allows customising output style
...
* ``rtoml`` is a Python wrapper for the Rust project ``toml-rs`` and hence has
  ...
  In addition, it does not support TOML 1.0.
...
to output plain Python types like ``str`` and ``dict``, and complicating the
design of the API.
...
exclusion for now, and potentially revisit this again later.
...
functions. If we agree that consistency with other stdlib modules is desirable,
...
However, TOML-facing users may include non-developers who are not aware of
the limits of double-precision float, or need greater precision.
Using ``decimal.Decimal`` allows users extra precision not promised by the
TOML format, which in the author of ``tomli``'s experience is particularly
useful in scientific and financial applications.
...
`API incompatibilities <Appendix A_>`_, even if they pin their dependency
versions.
...
package and repurpose it for backport of the standard library module,
we would still break users on new Python versions that include the standard
library module, regardless of whether they have pinned the version of the
current ``toml`` package. This is unfortunate, since pinning
...
#. Different first argument of ``toml.load``
   ...
   * Allowing passing of paths is inconsistent with
