...
For example, some libraries (e.g. ``typing.TypedDict``, dataclasses)
...
Language Reference:
    ...
    preferably be set to ``None``, rather than set to a function that returns an
    empty dict.
...
* If the annotations refer to free variables, the closure will
  be the appropriate tuple containing values for free variables.
