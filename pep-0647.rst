...
::
    ...
    @type_guard(List[str])
    is_str_list(val: List[object]) -> bool: ...
...
Strict type narrowing enforcement (i.e. requiring that the type specified
...
use cases for this functionality. For example, the ``is_str_list`` example
above would be considered invalid because ``List[str]`` is not a subtype of
``List[object]`` because of invariance rules.

One variant that was considered was to require a strict narrowing requirement
...
us to spell out all of the subtle ways in which the rules differ and under
...
parameter to be used as the value being tested for narrowing. Neither the
TypeScript language authors nor the author of this PEP could recall seeing any
examples in TypeScript where the parameter being tested was not the first
parameter. For this reason, it was decided unnecessary to burden the Python
implementation of user-defined type guards with additional complexity to
support a contrived use case. If such use cases are identified in the future,
there are ways the TypeGuard mechanism could be extended. This could involve
the use keyword indexing, as proposed in PEP 637.
...
of ``self`` or ``cls`` is required, these values can be passed as explicit
arguments to a type guard function.
