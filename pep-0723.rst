...
This PEP defines a metadata comment block format inspired loosely by
`reStructuredText Directives`__ [2]_.
...
* ``dependencies``: A list of strings that specifies the runtime dependencies
  of the project. Each entry MUST be a valid :pep:`508` requirement.
...
project that embeds their equivalent of ``pyproject.toml``, which is called
``Cargo.toml``:
