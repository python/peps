...
scripts that is required for proper runtime execution.
...
to learn a new format. More likely, even, is that tooling will eventually support
this transformation with a single command.
...
allowed in a future PEP that standardizes how backends are to build distributions
from single file scripts.
...
beginning of lines so would precisely match the contents of a ``pyproject.toml``
file. It is also is easier for tools to parse and modify this continuous block
of text which was
`one of the concerns <https://github.com/epage/cargo-script-mvs/blob/main/0000-cargo-script.md#embedded-manifest-format>`__
...
With this approach there is the potential to clutter scripts with many variables
as new extensions get added. Additionally, intuiting which metadata fields
correspond to which variable names would cause confusion for users.
