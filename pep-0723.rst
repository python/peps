...
One important thing to note is that the metadata is embedded in a doc-comment.
`Other syntaxes`__ are under consideration, including using attributes which
are somewhat like a syntatically recognized equivelant of dunder variables,
with the key difference between Rust's choice and this PEP being that any valid
Rust syntax will be allowed, requiring one of the Rust syntax parsers to work
with it, like `syn`__
...
We argue that our choice, in comparison to the doc-comment approach, is easier to read
