...
TODO: Do we write ``match`` statement or just match statement?
...
A literal pattern succeeds if the target value compares equal to the
...
The single underscore (``_``) is not considered a ``NAME``.  It is
treated specially as a `wildcard pattern`_.

A capture pattern always succeeds.  It binds the target value to the
...
target value (using the `==` operator).
...
TODO: This is how far I got.  Need to take a break.
