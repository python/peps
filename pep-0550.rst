...
* When a coroutine object is instantiated, its ``__logical_context__``
  is initialized with an empty Logical Context.

* Coroutine's ``.send()`` and ``.throw()`` methods are modified as
  follows (in pseudo-C)::

    if coro.__logical_context__ is not None:
        tstate = PyThreadState_Get()

        tstate.execution_context.push(coro.__logical_context__)
        ...
        try:
            ...
            coro.__logical_context__ = tstate.execution_context.pop()
    else:
        # Perform the actual `Coroutine.send()` or
        # `Coroutine.throw()` call.
        return coro.send(...)
...
* When Python interpreter sees an ``await`` instruction, it inspects
  the ``__logical_context__`` attribute of the coroutine that is about
  to be awaited.  For ``await coro``:

  * If ``coro.__logical_context__`` is an empty ``LogicalContext``
    object that ``coro`` was created with, the interpreter will set
    ``coro.__logical_context__`` to ``None``.

  * If ``coro.__logical_context__`` was modified by Python code, the
...
Generators, similarly to coroutines, have a ``__logical_context__``
attribute, which is set to an empty Logical Context when created.
...
not attempt to control their ``__logical_context__``.
...
Similarly to coroutines, generators with ``__logical_context__``
set to ``None`` simply use the outer Logical Context.
...
* ``uint64_t PyInterpreterState->context_key_deallocs``: every time
  a ``ContextKey`` is GCed, all Execution Contexts in all threads
  will lose track of it.  ``context_key_deallocs`` will simply
  count all ``ContextKey`` deallocations.

* ``uint64_t PyThreadState->execution_context_ver``: every time
  a new key is set, or an existing key is updated, or the stack
    ...
    class ContextKey:
        ...
        def get(self):
            ...
            if (self.last_tstate_id == tstate.unique_id and
                ...
                self.last_deallocs ==
                    tstate.iterp.context_key_deallocs):
...
Because adding and deleting keys from LogicalContext is a very common
operation, we would not be able to always use the optimized
``dict.copy()`` for LogicalContext, frequently resorting to use the
