...
which Python package indexes were used to install Python distributions as well
as digests of these Python distributions.
...
installing a Python distribution is available in the PR to pip
`pypa/pip#11865`_. It reuses the already available implementation for the
`direct URL data structure <direct_url_data_>`_ to provide
``provenance_url.json`` metadata file for cases when ``direct_url.json`` is not
created.

A prototype called `pip-preserve <pip_preserve>`_ was developed to
demonstrate creation of ``requirements.txt`` file considering ``direct_url.json``
and ``provenance_url.json`` metadata files.  This tool mimics the ``pip
freeze`` functionality, but the listing of installed packages includes also
hashes of Python distributions.
...
.. _direct_url_data: https://packaging.python.org/en/latest/specifications/direct-url-data-structure/
