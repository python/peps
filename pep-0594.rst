...
* According to David Beazley [5]_ the ``wave`` module is easy to teach to
  ...
  powerful and highly motivating exercise for a 9-yearl-old aspiring developer.
...
modules depend on `audioop`_ for some operations. The byteswap operation in
...
206 considers the module as *poorly designed and now near-impossible
to fix*.
