...
Even then, it should not be promoted as the "one obvious way".  Guido
`said it best
<https://mail.python.org/archives/list/python-dev@python.org/message/FM66WWN5YFCMRGMRWGHACOXLQYTWNXE2/>`_:

    Apparently dict(x, \*\*y) is going around as "cool hack" for "call
    x.update(y) and return x". Personally I find it more despicable
    than cool.
...
Note that the ``dict`` implementations of the ``copy`` and ``update``
methods are used.
