...
* ``stacklevel``: The number of stack frames to skip when emitting the warning.
  Defaults to 1.

If the decorated object is a class, the decorator wraps the ``__new__`` methods
such that instantiating the class raises a warning. If the decorated object is a
...
``__deprecated__`` on the object it returns.
