...
removal of various long-deprecated ``unittest`` features had to be
...
* An object may implement a :data:`typing.Protocol`, but only if deprecated
...
that type checker provide configuration options that cover both use cases.
...
``DeprecationWarning`` at runtime. However, this would raise a number of
thorny issues:
...
* 38 deprecations that are not easily detected in the type system (for
