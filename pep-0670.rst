...
Currently, most macros accepting pointers cast pointers arguments to the
expected types. For example, in Python 3.6, the ``Py_TYPE()`` macro
casts its argument to ``PyObject*``::

    #define Py_TYPE(ob) (((PyObject*)(ob))->ob_type)

The ``Py_TYPE()`` macro accept ``PyObject*`` type, but also any pointer
type like ``PyTupleObject*`` or ``PyDictObject*``.
...
The cast is kept for other pointer types like ``void*``. Removing a cast
to ``void*`` would emit a new warning if the function is called with a
variable of ``const void*`` type. For example, the *data* argument of
the ``PyUnicode_WRITE()`` accepts ``const void*`` type and then casts
*data* to ``void*`` type.
...
The PEP is design to avoid C API incompatible changes.

Only C extensions targeting explicitly the limited C API version 3.11
...
* Version 2: No longer remove return value, remove arguments cast from
  the limited C API.
