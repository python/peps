...
``Interpreter(id)``::
   ...
   run(source_str, /, **shared):
      ...
      only pipes (see "add_pipe()" below) are supported.
   ...
   add_pipe()::
      ...
      Create a new pipe and return the two ends: (recv, send).  These
      correspond to a "RecvPipe" and a "SendPipe", respectively.  Both
      ends of the pipe are supported "shared" objects (i.e. may be
      safely shared by different interpreters.  Thus they may be passed
      as keyword arguments to "Interpreter.run()".

      The current interpreter (where "add_pipe()" was called) and the
      target (on which "add_pipe()" was called) are associated with the
      pipe.  No other interpreters may use the pipe.

      Either end of the pipe must be closed before it will be destroyed.
      Once the pipe is closed it will no longer be tied to either
      interpreter.

   list_pipes():

      Return a list of all open pipes associated with the interpreter.

API for sharing data
--------------------

The mechanism for passing objects between interpreters is through pipes.
They are created calling ``Interpreter.add_pipe()``.

``RecvPipe(id)``::

   The receiving end of a pipe.  An interpreter may use this to receive
   objects from another interpreter.  At first only bytes and None will
   be supported.

   id:

      The pipe's unique ID.

   __next__():

      Return the next object from the pipe.  If none have been sent
      then block.

   recv():

      Return the next object from the pipe.  If none have been sent
      then wait until the next send.  If the pipe has been closed
      then EOFError is raised.

   recv_nowait(default=None):

      Return the next object from the pipe.  If none have been sent
      then return the default.  If the pipe has been closed
      then EOFError is raised.

   close():

      Mark the pipe as closed.  This will cause subsequent (or current)
      send() and recv() calls to raise EOFError.  The pipe will be
      destroyed (i.e. garbage collected) after it has been explicitly
      closed.  This is necessary because pipes are managed by the
      Python runtime and not by either interpreter.

      This operation is idempotent.  Return True if the pipe was still
      open and False otherwise.


``SendPipe(id)``::

   The sending end of a pipe.  An interpreter may use this to send
   ...
   send(obj):
       ...
       Send the object to the receiving end of the pipe.  Wait until
       the object is received.  If the pipe does not support the object
       then TypeError is raised.  Currently only bytes and None are
       supported.

   send_nowait(obj):

       Send the object to the receiving end of the pipe.  If "block" is true then block
       until the object is popped off.  If the pipe does not support
...
::
   ...
   interp = interpreters.create()
   r1, s1 = interp.add_pipe()
   r2, s2 = interp.add_pipe()
   def run():
       interp.run("""if True:
           fd = int.from_bytes(
                   reader.recv(), 'big')
...
support only one type of objects for shared state: the pipes provided
by ``Interpreter.add_pipe()``.  Pipes, in turn, will carefully manage
...
pipes, which do away with the need for explicit synchronization.  If
