...
C-API, along with new (very basic) functionality for sharing data
...
support, the module will also provide a mechanism for sharing data
between interpreters.  This mechanism centers around "channels",
which are similar to queues and pipes.
...
Many extension modules do not support use in multiple interpreters yet.
The maintainers and users of such extension modules will both benefit
when they are updated to support multiple interpreters.  In the meantime,
users may become confused by failures when using multiple interpreters,
which could negatively impact extension maintainers.
See `Concerns`_ below.
...
interpreters become more isolated, e.g. after GIL removal.
...
Using multiple interpreters is less useful without a mechanism for
sharing data between them.  Sharing actual Python objects between
interpreters, however, has enough potential problems that we are
avoiding support for that here.  Instead, only minimum set of types
will be supported.  Initially this will include ``None``, ``bytes``,
``str``, ``int``, and channels.  Further types may be supported later.
...
multiple interpreters, under `Extending Python <extension-docs_>`_.
The page will include the following information:

* a summary about multiple interpreter support (similar to the same
  in the new ``interpreters`` module page and in the C-API docs)
...
interpreters do not share state.  Data sharing is restricted to
channels, which do away with the need for explicit synchronization.
If any sort of opt-in shared state support is added to CPython's
interpreters in the future, that same effort can introduce
...
of multiple interpreters.  Such types include pipes (like unbuffered
channels, but one-to-one) and queues (like channels, but more generic).
See below in `Rejected Ideas`_ for more information.
