...
* add ``PyTypeObject.tp_shared``
* add C-API for "cross-interpreter" data
...
Normally interpreter isolation prevents sharing data (or even objects)
between two different interpreters.  This proposal includes a mechanism
by which an object's data (and potentially the object) may be shared.
This includes the new ``PyTypeObject.tp_shared`` and a number of
related functions.
...
Additionally, this proposal introduces a C-API by which a type's instances
(or their underlying data) may be shared.  This mechanism may be used to
facilitate more advanced sharing capabilities, as demonstrated by the
"channels" that were formerly a part of this PEP.  This mechanism is also
used for objects passed in to the "shared" arg of ``Interpreter.run()``.

The following types are added:

* ``typedef struct {} PyCrossInterpreterData``
* ``typedef PyObject *(*xid_newobjectfunc)(PyCrossInterpreterData *)``
* ``typedef void (*xid_freefunc)(void *)``
* ``typedef int (*crossinterpdatafunc)(PyThreadState *, PyObject *, PyCrossInterpreterData *)``

The specific ``PyCrossInterpreterData`` fields::

    struct {
        // data is the cross-interpreter-safe derivation of a Python object
        // (see PyObject_GetCrossInterpreterData).  It will be NULL if the
...
The following functions are added to the public (not limited) API::

    /* Used to initialize a PyCrossInterpreterData value. */
    ...
    int PyCrossInterpreterData_InitWithSize(
            ...
            xid_newobjectfunc);
    /* Used to finalize a PyCrossInterpreterData value. */
    void PyCrossInterpreterData_Clear(
            PyInterpreterState *, PyCrossInterpreterData *);

    /* Verifies that an object is shareable. */
    int PyObject_CheckCrossInterpreterData(PyObject *);
    /* Populates a PyCrossInterpreterData value for the given object. */
    int PyObject_GetCrossInterpreterData(PyObject *, PyCrossInterpreterData *);
    /* Returns an object that may be used safely in the current interpreter. */
    PyObject * PyCrossInterpreterData_NewObject(PyCrossInterpreterData *);
    /* Indates that the current interpreter is no longer using the object
       returned by PyCrossInterpreterData_NewObject(). */
    int PyCrossInterpreterData_Release(PyCrossInterpreterData *);

Note that the above C-API has already been in use as a private API
for multiple releases.

The final addition to the C-API is a new ``PyTypeObject`` field: "tp_shared".
The field allows a type to dictate how its instances may be shared.  If set,
it must be a valid ``crossinterpdatafunc``.
