...
Move private functions exposing implementation details to the internal C
API.

Convert macros to static inline functions.

Make structures become opaque: structures members of the C API must only
be accessed with getter and setter functions.

``Py_TYPE()``, ``Py_REFCNT()`` and ``Py_SIZE()`` can not be used as
l-value anymore: new functions ``Py_SET_TYPE()``, ``Py_SET_REFCNT()``
and ``Py_SET_SIZE()`` must be used instead.

A new ``pythoncapi_compat.h`` header file is distributed with Python to
provide new Python functions to old Python versions.
...
parts are already implemented. The `Process to introduce incompatible
changes in the C API`_ didactes the rythm.
...
* (**In Progress**) Make structures like ``PyObject`` or ``PyTypeObject`` opaque.
...
provides new functions to old Python versions. Example::
