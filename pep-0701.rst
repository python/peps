...
The specifics to how (or if) the ``tokenize`` module will emit this tokens or some others and what
...
* Many people find quite reuse withing the same string confusing and hard to read. This is because
...
* Some users have raised that quote reuse may break some lexer and syntax highlighting tools that rely
  ...
  working or will break them altogether (as for instances regular expressions cannot parse arbitrary nested
...
* Many languages that allow similar syntactic constructs (normally called "string interpolation") allow quote
  ...
  The fact that many languages allow quite reuse can be a compelling argument in favour of allowing it in Python. This
  is because it will make the language more familiar to users coming from other languages. 

* As many other popular languages allow quite reuse in string interpolation constructs, this means that editors
...
* One advatange of allowing quote reuse is that it composes cleanly with other syntax. Sometimes this is referred
  as “referential transparency”. An example of this is that if we have ``f(x+1)``, assuming a is a brand new variable, it
  should behave the same as ``a = x+1; f(a)z``. And vice versa.  So if we have::
...
* Limiting quote reuse will raise considerable the complexity of the implementation of the proposed changes. This is because
  that will force the parser to have the context that is parsing an expression part of an f-string with a given quote in order
  to know if it need to reject an expression that reuses the quite. Carrying this context around is not trivial in parsers that
...
has been made to get a sense of how the community feels about this aspect of the PEP. The authors of this
...
#. Although we think the readability arguments that have been raised against
   ...
   complexity and maintainance of parsing f-strings in CPython and this will not
