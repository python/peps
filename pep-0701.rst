...
* ``FSTRING_START``: This token includes f-string character (``f``/``F``) and the opening quote(s).
...
This PEP does not introduce any backwards incompatible syntactical or semantical changes
to the Python language. However the ``tokenize`` module (a quasi-public part of the standard
...
#. We have decided not to allow for the time being using escaped braces (``\{`` and ``\}``)
