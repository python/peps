...
* Object allocations and method wrappers aren't free, and tracing functions
  ...
  regular operation as we can possibly make them. It also means that optimising
  compilers and interpreters can safely assume that untraced code will only
  mutate function local variables at points where the compiler can see name
  binding operations - they only need to switch back to more conservative
  assumptions if their tracing mode behaviour mirrors the reference
  implementation in allowing mutation via the ``frame.f_locals`` attribute.
