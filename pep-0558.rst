...
As noted in [9]_, the implicit call to ``PyFrame_LocalsToFast()`` in the
...
``PyFrame_LocalsToFast()`` could be dropped, as the snapshot would only be
accessible from extension module trace hooks, and they already need to make
their own calls to ensure the snapshot is up to date before relying on it.
