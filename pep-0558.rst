...
* any call to ``PyFrame_GetPyLocals()`` or ``PyFrame_GetLocalsAttribute()``
  for the frame
* any call to ``PyEval_GetLocals()`` or the Python ``locals()`` builtin while
...
should be changed to use ``PyFrame_GetPyLocals(PyEval_GetFrame())`` instead.
...
are called in a loop, calling ``locals()`` once before the loop will give the
same semantics and performance characteristics as the status quo, whereas
relying on the implicit default would create a new snapshot on each iteration).
