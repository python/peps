...
Postponed to Python 3.10
========================

The draft implementation still needs a lot of enhancements to make the step
from "proof of concept" to "ready for merge", and there is some discussion
still to be resolved around adding 5 new functions to the stable C ABI, and
a further 6 to the public (or semi-public) CPython C API.

This work isn't going to be completed in time for Python 3.9b1, so the target
Python version has been pushed back to 3.10 in 2021 (however, the goal will
be to have the PEP resolved relatively early in the 3.10 alpha cycle, ideally
before 3.10a1).
