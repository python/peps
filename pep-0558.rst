...
Instead of being a direct reference to the dynamic snapshot historically
returned by ``locals()``, ``frame.f_locals`` will be updated to instead return
a dedicated fast locals proxy type that has an internal reference back to the
underlying frame object. As with the existing function level ``f_locals``
attribute, this proxy object will only be created on demand, rather than being
present by default.

``__getitem__`` operations on the proxy will read the corresponding fast local
or cell reference on the underlying frame rather than storing a separate
mapping from keys to values.

Similarly, ``__setitem__`` and ``__delitem__`` operations on the proxy will
directly affect the corresponding fast local or cell references on the
underlying frame.

Attempting to set or delete a key through the proxy that isn't defined as a
local variable or closure reference on the underlying frame will raise
``KeyError``. Attempting to delete a key that has already been deleted from
the underlying frame will also raise ``KeyError``.
...
frames if it doesn't already exist.
...
function level fast locals proxy bypass logic to be encapsulated entirely inside
