...
Today, there is an |overrides-library|_ that provides decorators
...
to correctly work with multiple decorators, because successfully setting
the ``__override__`` field on the final output requires understanding
...
.. |overrides-library| replace:: ``overrides`` library

.. _overrides-library: https://pypi.org/project/overrides/
