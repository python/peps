...
This proposal is part of an ongoing effort to remove from the language
the redundancy of the 3-item exception representation, a relic of earlier
Python versions which is now confuses people who are learning and using
the language, while adding complexity and overhead to the interpreter.
...
esoteric ones will conttinue to be called with three arguments. This
...
for evolving the signature of ``__exit__``.

In the case of the :mod:`py3.11:traceback` module's API, evolving the
functions to have a hybrid signature is relatively straighforward and
...
the callback is excpecting, by marking it with an additional attribute or
...
function is not ideal, but it is an option.
...
While discussing the real-world impact of the problem of this PEP, it is
