...
- an installation within a feature release series can be updated to a later
  maintenance release within the same series without requiring reinstallation
...
for all X.Y.z feature releases around 18 months after X.(Y+1).z is released.
...
tested against the latest micro feature release, but typechecked against the
oldest still deployed micro feature release.
...
from the 2019-10 release of Python 3.8.0 and a final Python 3.9.x micro feature
release in 2021-10 evenly between pre-release development and subsequent
micro feature releases.
...
months of micro feature releases:
...
"Python 3.10" (code incorrectly assuming that the micro version will always
...
major version number, rather than both the major and micro version).
