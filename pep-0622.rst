...
* For the most commonly-matched built-in types (specifically:
  ``bool``, ``bytearray``, ``bytes``, ``complex``, ``dict``,
  ``float``, ``frozenset``, ``int``, ``list``, ``set``, ``str``,
  ``tuple``, and ``type``), a single
  ...
  against the proxy itself.  This creates behavior that is useful and
  intuitive for these objects:
