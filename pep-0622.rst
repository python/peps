...
unpacking (e.g., ``a, b = value``). A ``match`` statement compares
its subject to several different “shapes” until one is found that
fits. Like in sequence unpacking, components of the matched value
can be captured in variables.
  
In addition to specifying the shape of a sequence to be unpacked,
patterns can also specify the shape to be:
