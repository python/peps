...
y]`` to be a double subscript. It then encounters the colon which might
mean that either this is an annotated assignment or a match statement.
When it sees the following ``NEWLINE``, it has to backtrack, since an
annotated assignment expression expects something other than a ``NEWLINE``
in this case. The parser then backtracks to the start of the line and finds
that ``match`` is a soft keyword allowed in this position. It then considers
``[x, y]`` to be a list expression. The newline then is just what the parser
expected, and the parse succeeds.
