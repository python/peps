...
the matching expression (same as for literals)::
...
Note that it is not possible for unqualified names (such as ``BLACK``
...
* Introduce a special symbol, for example ``.``, ``$``, or ``^`` to
  ...
  leading-dot rule::
    ...
    match [value, 42]:
        case [.FOO, x]:  # This would not be matched
            ...
        case [BAR, x]:  # This would be matched
            ...
  ...
  namespace (considered by the authors to be one honking great idea)::
    ...
    match [value, 42]:
        case [Consts.FOO, x]:  # This would not be matched
            ...
        case [BAR, x]:  # This would be matched
            ...
