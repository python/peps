...
.. csv-table:: Table 1: Audit Hooks
   ...
   str."
   ``_PyObject_GenericSetAttr``,``type.__setattr__``,"``(type, attr_name)``","
   Detect monkey patching of types. This event is only raised when the object is
   an instance of ``type``."
   ``type_set_bases``,``type.__bases__``,"``(type,new_bases)``","Detect attempts
   to modify the bases of a type."
   ``object_set_class``,``object.__class__``,"``(old_class, new_class)``","
   Detect attempts to change the class of an object."

TODO - more hooks in ``_socket``, ``_ssl``, others?
* code objects
* type objects (e.g. type_set_bases, object_set_class)
* function objects
