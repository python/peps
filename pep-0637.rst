...
One apparently rational choice would be to pass now value at all, but
unfortunately will not work well with the ``__setitem__`` dunder, as a
positional element for the value is always passed. This makes it impossible to
have a keyword only dunder, and opens up the question of what to pass for the
index position::

    obj[k=3] = 5  # would call type(obj).__setitem__(?, 5, k=3)

A potential hack would be to specify a default for the ``index``, but this
...
which seems ugly and confusing. We must therefore accept that some form of
sentinel index must be passed by the python implementation when the
``obj[k=3]`` notation is used.
...
The current choice is option 1, that is, to pass an empty tuple::

    type(obj).__getitem__((), k=3)

therefore making ``obj[k=3]`` and ``obj[(), k=3]`` degenerate.

The tuple has been chosen because:
...
``None`` was also proposed but NumPy uses it to indicate inserting a new
axis/dimensions (there's a ``np.newaxis`` alias as well)::
...
The final conclusion is that we favor the following series::
