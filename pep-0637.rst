...
where the index (first argument) contains the set value (normally second argument, but first
if no index is specified). This is extremely deceiving and error prone.
...
A proposed hack would be to specify a default for the ``index``, but this
