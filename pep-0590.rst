...
Most new API is private in CPython 3.8, with plans to finalized and
made public in Python 3.9.
...
The API may change (based on onsumer feedback) in Python 3.9, where we expect
...
If ``_Py_TPFLAGS_HAVE_VECTORCALL`` is not set, then ``tp_vectorcall_offset`` must be zero.
...
``PyObject_Vectorcall`` and ``Py_TPFLAGS_HAVE_VECTORCALL``, with the former
names provided using macros.
