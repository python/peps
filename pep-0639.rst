...
package author construct a one which expresses their intent.
...
and either defers implementation on PyPI to a later PEP, and gives
...
accessing installed licenses programmatically would be more non-trivial, and a
...
As discussed previously, file-level notices out of scope for this PEP, and the
...
- In the `Setuptools <#setuptoolssdist_>`_ and `Wheel <#wheels_>`_ projects,
  ...
  location in in a source distribution/sdist, and in the ``.dist-info``
...
- The `JavaScript Bower <#bower_>`_ ``license`` field is either a single or
