...
is simpler and less ambiguous for:
...
- Specify a ``License-File`` field which is already used by ``wheel`` and
  ``setuptools`` to include license files in built distributions.

- Define how tools can validate license expressions and report warnings to
...
A text string that is a path, relative to ``.dist-info``, to a license file.
...
- Classifiers ``License :: GUST*`` have no mapping to SPDX license identifiers
...
- The section `Mapping Legacy Classifiers to New License expressions`_ provides
...
License Files in Wheels and Setuptools
--------------------------------------
...
- In wheels [#wheels]_ license files are automatically added to the ``.dist-info``
  ...
  as ``LICENSE*`` and ``COPYING*``). Alternatively a package author can specify
  a list of license file paths to include in the built wheel in the ``license_files``
  field in the ``[metadata]`` section of the project's ``setup.cfg``.
  Previously this was a (singular) ``license_file`` file attribute that is now
  deprecated but is still in common use. See [#pipsetup]_ for instance.
...
in ``wheel`` (with the ``license_file`` and ``license_files`` ``setup.cfg``
``[metadata]`` entries) and in the ``setuptools`` ``license_file``
``setup()`` argument.
...
- Flit [#flit]_ recommends to use classifiers instead of ``License``
  (as per the current metadata spec).
