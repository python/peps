...
Defining module ``__setattr__`` or ``__delattr__`` only affect lookups made
...
To use a module global with triggering ``__setattr__`` (or ``__delattr__``)
function one can access it via ``sys.modules[__name__]`` within the module
code:
