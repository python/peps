...
repositories conforming to the Simple Repository API do not serve a mapping
from import names to package names (see also
...
the same import name as an existing obscure package.
...
However, it is not clear where to best attach the information in the case of
multiline imports. The following::
