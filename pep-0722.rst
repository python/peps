...
files, etc. When used to create scipts, Python code is typically stored as a
...
the identifying chacaters ``##``. Whitespace is not allowed before the
...
Within a metadata block, whitespace at the start and end of each line is
ignored, and blank lines are ignored. The first line of a metadata block is
...
There MUST only be a single ``Script Dependencies`` block in the file. Tools
consuming dependency data MAY simply process the first such block found. This
avoids the need for tools to process more data than is necessary. Stricter tools
MAY, however, fail with an error if multiple script dependency blocks are
present.
...
embed such a requirement into a TOML structure is an additional complexity - and
it is important to remember that the goal here is to make using 3rd party
