...
``TypeVar`` \ s but uses a ``list`` or ``tuple`` of types instead of a
single type.
...
``TypeVar`` \ s but uses an unpacked tuple of types instead of a single type.

.. code:: py

   DefaultTs = TypeVarTuple("DefaultTs", default=*tuple[str, int])
