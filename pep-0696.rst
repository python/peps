...
.. note::
   ...
   - :ref:`696-scoping-rules` not allowing usage of ``TypeVarLikes``
     from outer scopes
   - Multiple ``TypeVarTuple``\s not being able appear in the same type
     parameter lists to a class as defined in
     :pep:`646#multiple-type-variable-tuples-not-allowed`
   - Function defaults not being supported

   These reasons leave no current valid location for a
   ``TypeVarTuple``, hence, they are not supported.
