...
- The classes ``TypeVar``, ``ParamSpec``, and ``TypeVarTuple`` would
  all expose the type passed to default which would be available as a
  ``__default__`` attribute with the default being ``None`` if no
  argument is passed and ``NoneType`` if ``default=None``.
