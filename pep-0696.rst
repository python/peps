...
Note
^^^^
``TypeVarTuple``\s as defaults are not supported due to
:ref:`scoping-rules` not allowing usage of ``TypeVarLikes`` from outer
scopes, multiple ``TypeVarTuple``\s cannot appear in the same
type parameter lists to a class and function defaults not being
supported.
...
Allowing ``TypeVarLike``\ s to be used in function signatures
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

A previous version of this PEP allowed ``TypeVarLike``\ s to be used in
function signatures. This was removed for the rationale described In
:ref:`function-defaults`. Hopefully, this can be added in the future if
