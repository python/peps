...
which act as defaults for a type parameter when none is specified or
...
fails to solve a constraint for a type or the a type parameter to a
generic is missed out but another is specified.
...
When using another ``TypeVarLike`` (``T1``) as the default, the default
for the ``TypeVarLike`` (``T2``) the following rules apply.
...
``T2``'s constraint must either contain the bound of ``T1`` or the
...
``default`` when the first parameter is in scope.
...
.. code:: py
   ...
   def foo(x: ListDefaultT): ...
   def bar(y: T, x: ListDefaultT): ...
