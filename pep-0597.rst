...
But the WSL changes the active code page to 65001, and  ``python.exe`` on
Windows can be executed from the WSL.  So ``python.exe`` executed from the
legacy console and from the WSL cannot read text files written by each other.
...
* If the script assumes ``latin1`` or ``cp932``, use ``encoding="latin1"``
