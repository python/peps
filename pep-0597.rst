...
For example, these commands do not cause mojibake regardless the active code
page::
...
There may be scripts relying on the locale encoding or active code page which
is not UTF-8.  They must be rewritten to specify ``encoding`` explicitly.
