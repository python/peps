...
* STOP_ITERATION -- An artificial ``StopIteration`` is raised, see below.
...
Only local events can be set on a per code object basis.
...
* ``def set_local_events(tool_id: int, code: CodeType, event_set: int)->None``
* ``def insert_marker(tool_id: int, code: CodeType, offset: int)->None``
* ``def remove_marker(tool_id: int, code: CodeType, offset: int)->None``
