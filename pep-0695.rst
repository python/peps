...
- If a ``def965`` scope is immediately within a class scope, or within another
  ...
  by contranst, cannot access names defined within an enclosing class scope.)
- The following constructs are disallowed anywhere within a ``def695`` scope, though
  they may be used within regular functions nested inside a ``def695`` scope:
