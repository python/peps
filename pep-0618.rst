...
(wrongly) imply some notion of "equality" between the zipped items::
    ...
    >>> zip([1, 2, 3], [1.0, 2.0, 4.0], equal=True)
...
unequally-sized inputs (for example, when dealing with infinite
iterators).  Likely, this backward-incompatible change would break
more code than it "fixes".
