...
It seems that the main motivation driving this proposal is that
``zip_longest``, a cousin of ``zip``, already exists in ``itertools``.
However, ``zip_longest`` is really another beast entirely: it takes on
the responsibility of filling in missing values, a problem neither of
