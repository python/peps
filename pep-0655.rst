...
``total=False`` TypedDict to mark them as required, and
``NotRequired[...]`` which can be used on individual items of a regular
(``total=True``) TypedDict to mark them as potentially-missing.
...
It is an error to use ``Required[...]`` on an item of a TypedDict whose
totality is true:

::

   class Movie(TypedDict):   # implicitly total=True
       title: Required[str]  # error: Required[...] is only usable when total=False
...
It is an error to use ``NotRequired[...]`` on an item of a TypedDict
whose totality is false:

::

   class Cell(TypedDict, total=False):
       value: NotRequired[object]  # error: NotRequired[...] needs total=True
