...
rather than giving it its own separate module.  This idea, or some variation on
it, is reasonable, but in the end was ultimately rejected.
...
with a separate documentation page.  It may also make sense to nest
``zoneinfo`` as a lazily-imported module under ``datetime``, as
``datetime.zoneinfo``, but this may either constrain the implementation of the
module or require a potentially-uncomfortable refactoring in ``datetime``, and
it does not give a large tangible benefit beyond some increased
discoverability.
