...
Python should guarantee that when tracing is turned on, "line" tracing events are for *all* lines of code executed and *only* for lines of
...
After a frame has completed, either from returning or raising an exception,
...
Users of ``sys.settrace`` and associated tools should be able to expect that tracing events are
generated for all lines of code, only for actual code, and that the line number in ``f_lineno`` is correct.
...
Users and tools should be able to assume that line events are generated for all lines of code and only for lines of code.
They should also be able to expect that the frame's ``f_lineno`` attribute is always correct.

In order to make these guarantees, the ``co_lnotab`` attribute, in its current form,
...
To ensure that nothing breaks, the ``co_lnotab`` attribute will be lazily generated on demand.
...
* Whenever a line event would be generated during tracing, even if tracing is turned off,
...
The ``co_lines()`` method will return an iterator which yields triples of values,
each representing the line number of a range of bytecodes.
...
* The final range in the sequence with have a ``end`` equal to the size of the bytecode.
...
Historically the ``co_lnotab`` attribute held mapping from bytecode offset to line number.
It does not support bytecodes without a line number.
...
Tools that use ``sys.settrace`` will be unaffected, except in cases where sequence of "line" events they receive is more accurate.
...
When tracing, code should be a little faster as the new table format can be optimised for line number calculation speed.
Code with long chains of ``pass`` statements will probably become a bit slower.
