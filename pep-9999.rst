...
latest CPython pre-releases). In some cases, both may exist within the same
organisation for different purposes (for example, a stable Python in the build
environment and the most recent update for data scientists, or a stable Python
deployed to end user systems, and the most recent update available as an
option in a web service hosting environment).
