...
which option to expand as no further token of the program can be examinedto
disambiguate. As will be shown later in this document, the current LL(1)-based
grammar suffers a lot from thisscenario.

Also is relevant to note (as other sections of this document will deal with this
...
sentential form with itself as theleftmost symbol. For instance this rule:::
...
express some desired properties in the directly in the grammar and the lack of
...
difference is that the choice operator is ordered. This means that when writting::
...
PEG parsers are usually constructed as recursive descent parser in which every
...
(this means that they can consider an arbitrary number of tokensbefore deciding
...
instance in `bpo-26451: Excessivepeak memory consumption by the Python
...
considerably the maintainande burden.
...
AST pieces for a rule via grammar actions. Grammar actions are C expressions that
...
returned. 2. If not, a dummy name object gets returned.
...
automatically injected by the parser.
...
3.- Between Python 3.9 and Python 3.10, the old parser and related code (like the
    ...
    kill be kept LL(1) until the old parser is removed.
