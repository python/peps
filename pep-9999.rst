...
Sponsor: Chris Jerdonek <chris.jerdonek@gmail.com>
...
- A ``requested_revision`` field MAY be present to reference the
...
to use the ``vcs``, ``requested_revision`` and ``commit_id`` fields.
...
"Freezing" an environment
  ...
  to generate a higher fidelity output. In presence of a ``vcs`` direct URL reference,
  ...
  tools are encouraged to output both ``requested_revision`` and ``commit_id``.
...
essence of the abstract. Nick Coghlan suggested the direct_url name. Chris
Jerdonek made many insightful suggestions that helped clarifying several
aspects.
