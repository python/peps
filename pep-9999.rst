...
translating to 11% speedup for one sample benchmark derived from real-world code
...
Now including the outer ``lst`` variable as a local, and eliminating the
synthetic ``.0``.
...
This PEP has a reference implementation in the form of `a PR against CPython main
...
timeit -s 'l = [1]' '[x for x in l]'`` 1.96x faster than ``main`` branch (in a
