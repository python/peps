...
-----------------------------------
By default the attrs library determines the order of fields within a
class in the same way that dataclasses do. This ordering can differ
from Method Resolution Order (MRO) in certain diamond-shaped multiple
inheritance scenarios. Attrs users can get MRO behavior in those
scenarios by specifying ``collect_by_mro=True``. This proposal does
not support any field ordering other than that used by dataclass.
