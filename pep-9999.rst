...
of code with 100% test coverage and passes all tests in a test suite `proposed to
be the official TOML compliance test suite <https://github.com/toml-lang/compliance/pull/8>`.
...
objects, merging the documents into a single object). Doing this would be inconsistent
...
the ``toml`` package as an (incompatible) backport.
...
#. Different first argument of ``toml.load``
   ...
   * Allowing passing of paths (and lists of path-like objects, merging the documents
     into a single object) is inconsistent with other similar functions in the standard
     library.
   * Using ``SupportsRead[bytes]`` allows us to a) ensure utf-8 is the encoding used,
     b) avoid incorrectly parsing single carriage returns as valid TOML due to
