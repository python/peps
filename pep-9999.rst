...
* ``toml``.
  ...
  For more details on API differences, refer to this `discuss thread
...
If so, having a basic write API will minimise disruption for most affected
users.
...
While slightly opinionated, this was a recent change in ``tomli`` v1.2 to a)
...
name on PyPI, we could repurpose the PyPI package as a stdlib backport. This
would still potentially be breaking for users who have pinned current versions
of the ``toml`` package and have upgraded Python versions. However, based on
https://grep.app, relatively few users would be affected as long as we
include a basic write API.
