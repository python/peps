...
* ``default_factory`` or ``factory`` is an optional parameter that
  ...
  field. If ``default`` and ``default_value`` are both unspecified,
  the field is assumed to have no default value and must be provided a
...
``__init__`` method to convert the attribute's value to some other
desired value. We decided against supporting this as it would be
somewhat complex to implement and very specific to attrs. Converters
can still be used, but an explicit type annotation must be provided::

  @attr.s
  class C:
      x: int = attr.ib(converter=int)

This issue was discussed at length on the pyright GitHub site.
[#converters]_
