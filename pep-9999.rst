...
* Beta releases from the master git branch would occur every other month
  whenever there is no X.Y.0 release in the release candidate phase
* Prior to the release candidate phase, all newly added interfaces are
  considered provisional, and each new release may introduce new deprecation
  warnings (or escalate previous deprecation warnings to errors)
* Routine alpha releases are removed from the process (release managers may
  still choose to publish alpha releases prior to starting the beta release
  phase if they feel such releases are appropriate)
...
PEP 602 proposes that affected organisations simply switch to adopting every
second or third CPython release, rather than attempting to adopt every release,
but that creates its own set of new problems to be resolved, both practical (e.g.
deprecations would need to cover more than one release if we're expecting users
to routinely skip releases) and cultural (e.g. with a larger number of versions
in active use, there is a much higher chance that open source library maintainers
will receive bug reports that only occur on Python versions that they're not
using themselves).
...
* The final release would occur between 1 and 4 weeks after the final release
  candidate (depending if additional release candidates were needed after the
  second)
...
CPython alpha releases would no longer serve a clear purpose, and as such
would typically be skipped.

If they chose to, release managers would still be free to make one or more
alpha releases before entering the rolling beta release period. For example,
that would be one way of testing out new release management tooling.
...
encourage widespread operational use of beta releases, and we'd like to make it
clear that such binaries are only reliably portable to the exact version that
was used to build them (they may fail to load on even the very next alpha or
beta release).

To that end, this PEP proposes amending the extension module ``SOABI`` marker
for CPython pre-releases to use a "cpython_unstable-" prefix, and only switch
back to the regular "cpython-" prefix once the ABI for that particular version
has been frozen on entry to the release candidate stage.

(TODO: Link to https://github.com/ncoghlan/cpython/pull/3)
(TODO: What's the Windows equivalent of this?)
...
The "production ready, but subject to change" caveat also aligns well with the
"beta" term.
...
In odd-numbered years, PyCon US would naturally focus on release candidate
preparations, while the September core development sprints would focus on
closing out any final non-security bug fixes for the release series entering
its security-fix-only phase, the initial set of post-release fixes for the just
released series, and preparing for the first beta release in the next
release series.

In even-numbered years, the focus for both events would just be on the upcoming
