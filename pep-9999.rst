...
used in subscript expressions at runtime (e.g. ``func[str]``).
...
::
    ...
    TypeAliasA: TypeAlias = _T | "TypeAliasA[_T]"
...
::
    ...
    type TypeAliasA[T] = T | TypeAliasA[T]
...
::
    ...
    class A[T, *T]: ... # Syntax Error

    def func[T, **T](): ... # Syntax Error
...
::
    ...
    class A[T](BaseClass[T], param = Foo[T]): ...  # OK
...
::
    ...
    class A[T extends dict[str, int]]: ...  # OK

    class B[T extends "ForwardDeclaration"]: ...  # OK

    class C[T extends dict[str, V]]: ...  # Type checker error

    class D[T extends [str, int]]: ...  # Type checker error

    class E[T extends (str, int)]: ...  # Type checker error
...
::
    ...
    class A[AnyStr in (str, bytes)]: ...  # OK

    class B[T in ("ForwardDeclaration", bytes)]: ...  # OK

    class C[T in ()]: ...  # Type checker error

    class D[T in (str, )]: ...  # Type checker error

    class E[T in [str, bytes]]: ...  # Type checker error

    class F[T in (3, bytes)]: ...  # Type checker error

    class G[T in (list[S], str)]: ...  # Type checker error
...
::
    ...
    class A[T = int, *Ts = tuple[int, ...], **P = [int, str]]: ...
...
::
    ...
    class B[S = int, T]: ...  # Syntax error: T has no default argument
...
::
        ...
        class A[T extends float = int]: ...  # OK

        class B[T extends int = float]: ...  # Error: float is not a subtype of int

        class C[T in (str, float) = int]: ...  # OK

        class D[T in (str, float) = int]: ...  # Error: int is not one of (str, float)
...
::
    ...
    class A[V](dict[K, V]): ...  # Type checker error

    class B[K, V](dict[K, V]): ...  # OK
...
::
    ...
    A = (lambda T: class A(BaseClass[T]): ...)()
...
::
    ...
    class A[T]((x := BaseClass[T])):
        ...
    
    # x is not defined in this scope.

    class B((y := BaseClass[int])):
        ...
