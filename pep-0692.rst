...
names that it actually expects. All of this makes certain issues, such as a
helper function being updated with a new keyword argument that the public
function that uses the helper doesn't take into account, difficult to spot.
...
a widely used pattern. As a consequence, there has been a lot of discussion
around supporting more precise ``**kwargs`` typing and it became a feature that
would be valuable for a large part of the Python community -- the `mypy GitHub
issue 4441 <mypyIssue4441_>`__ links to a lot of real world cases that could
benefit from more precise ``**kwargs`` typing.

One more use case worth mentioning that ``**kwargs`` are also convenient for,
...
``__typing_kwargs_unpack__`` and it should return ``Unpack[self]``.
