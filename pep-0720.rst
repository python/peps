...
.. [2] We specifically mention *popular* workflows, because this is not
       ...
       is what we are refering to here. For clarity, the implementations we are
       refering to here could be described as *crossenv-style*.
...
the target system is the same as where the build is ocurring, so introspection
...
.. list-table::
   ...
   *  - .. code-block:: python
      ...
      - Python language version implementated by the interpreter.
...
particularities — scattered accross many places, and it often is difficult to
...
example is Android, where only the main executable and ``LD_PRELOAD``s are
considered to be ``RTLD_GLOBAL`` (meaning dependencies are ``RTLD_LOCAL``)
[10]_, which causes the ``libpython`` symbols be unvailable when loading the
extension.
...
installation/environment files are installed, acording to the table below.
