...
In a previous iteration of this proposal, AS patterns were named
walrus patterns and the syntax was ``capture_pattern := or_pattern``.
We decided to switch to ``or_pattern as capture_pattern`` for several
reasons:

- The new syntax means that all values flow from left to right.

- When using this in a class pattern, we disallowed combining the
  walrus syntax with keyword patterns, because ``Foo(a = b := c)`` was
  confusing (and because the same syntax is forbidden in expressions
  by PEP 572).  But ``Foo(a = c as b)`` is not as confusing, and can
  be allowed.  Similarly, we can allow unparenthesized AS patterns in
  mapping patterns.

- The walrus operator in expressions binds exactly the value found to
  its right.  But that was not the case here: to its right was a
  pattern, but the value bound is the subject to that pattern.
