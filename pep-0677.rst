...
We could alternatively use ``Def(int, str) -> bool``, where the
capital letter strongly suggests a type rather than a lambda. But
that would require a new keyword, and we do not see any benefits that
justify adding a new keyword or making the types more verbose than
...
functions, but the problem is that it makes callables with no arguments, which
are *very* common in typeshed, hard to write. Because ``()`` is not a legal
tuple they would have to be written as ``callable[(,), bool]`` which is awkward
enough that we dislike this idea.
