...
Reuse their names for alternative APIs
--------------------------------------

As described in "Alternative APIs" section, some APIs don't have
...
from private name to public and dperecated names.
...
* We have more public API set to maintain for rare use cases.
...
* It is breaking change because user may assume ``Py_UNICODE`` is a
  typedef of ``wchar_t`` but ``sizeof(wchar_t)`` is 2bytes on some
  platforms.
* We have more public API set to maintain for rare use cases.
...
We can replace ``Py_UNICODE`` to ``wchar_t``. Since ``Py_UNICODE`` is
...
objects using UTF-8, UTF-16, and UTF-32 codec, like "Replace
...
* Although Windows is the most major platform that ``wchar_t`` is
  widely used, these APIs need to create a temporary Unicode APIs
