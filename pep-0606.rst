...
PEP 605 proposes to address it by actively seeking to create a community of
...
Marking APIs as provisional offers a way to avoid that constraint, but actually
taking advantage of that freedom causes other problems.
...
where that approach doesn't apply)
...
With a 12-month release cadence each release branch will spend proportionally
more time in security-fix-only mode (~48 months out of ~60 vs the current ~42
out of ~60).
...
less time in security-fix-only mode (~36 months out of ~60 vs the current ~42
out of ~60).
...
by keeping the minimum gap between releases to 12 months.

PEP 605 aims to mitigates the potential negative impact on members of this group
by focusing its change proposals on the management of the pre-release period.
However, some users of the stable releases are likely to be unhappy with the
slower release cadence in that PEP, while also being unwilling to make the jump
to using the rolling beta release stream.
...
redistributors that take on the task of making builds available directly from
the CPython master branch.

Neither PEP 602 nor PEP 605 should directly affect this group, but the rolling
release stream proposal in PEP 605 aims to substantially reduce the barriers to
more users adopting this style of usage (except with the rolling beta stream,
rather than using the master branch directly).
...
by increasing the gap between full releases to 24 months, and retaining the
"beta" naming scheme for the rolling release stream (at least the Python 3.9
release cycle).
