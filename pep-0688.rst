...
others accept all buffers. This PEP proposes a special marker that
mutable buffers should expose to indicate that they are mutable.
...
Buffer classes that are always mutable or that may be mutable should add a
new class attribute, ``__mutable_buffer__``, set to ``True``. This attribute
serves as a marker to static type checkers and runtime ABC checks (see below).
The marker should be set even on classes that may be either mutable or immutable
depending on instance-specific behavior, such as ``memoryview``.
...
method; the latter requires both ``__buffer__`` and ``__mutable_buffer__``.
...
Use the ``bf_releasebuffer`` slot to indicate mutability
--------------------------------------------------------
