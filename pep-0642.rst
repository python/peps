...
Interaction with caching of attribute lookups in function local variables
-------------------------------------------------------------------------

The major change between this PEP and PEP 634 is the use of `?EXPR` for value
constraint lookups, rather than `NAME.ATTR`. The main motivition for this is
...
value patterns has an undesirable interaction with function local variable
assignment, where routine refactorings that would be semantically neutral for
any other Python statement introduce a major semantic change when applied to a
match statement.
...
With the syntax proposal in this PEP, constraint patterns would be similarly
tolerant of match patterns being refactored to use a function local variable
instead of an attribute lookup, with the following two statements being
...
in the way they would combine with the `=` as a keyword separator in class
...
unpacked value at this position equivalent to the given value? If not, don't
match")).
...
first ever syntax-sensitive statement where you can't transparently
...
as a general purpose non-binding wildcard marker.
...
prefix). "Truth testing" related use cases such as PEP 505's None-aware
operators would likely fare especially well on that front.
...
While writing `x is None` is a common (and recommended) practice, nobody litters
their `if-elif` chains with `x is True` or `x is False` expressions, they write
`x` and `not x`, both of which compare by value, not identity. Indeed, PEP 8
explicitly disallows the use "if x is True:" and "if x is False:", preferring
the forms without any comparison operator at all.
...
Requiring the use of constraint patterns for mapping pattern keys
-----------------------------------------------------------------
...
Also note that in PEP 634, arbitrary expressions are technically supported in
value patterns, they just require an awkward workaround where all the values to
match are specified in a helper class::

    # Arbitrary match targets with PEP 634's value pattern syntax
    class mt:
        value = func()
    match expr:
        case mt.value:
...
If static analysers can't follow certain kinds of dynamic checks, then they can
limit the permitted expressions at analysis time, rather than the compiler
restricting them at compile time.
...
Note: this still needs a reference implementation to confirm that the proposed
      ...
      constraint pattern definitions are correct)
