...
constraint expressions removes the restriction to only working with bound names
...
* the parser doesn't distinquish f-strings from other kinds of strings, so
  inferring an explicit equality constraint for f-strings happens by default
  when defining the match pattern parser rule for string literals
...
string literals, so the PEP instead opts to just allow them.
