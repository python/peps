...
On the PyPI top 5000 projects, only 13 projects (0.3%) are affected by 2
macro changes. Moreover, 22 projects just have to regenerate their
Cython code.
...
On these 13 projects, only 2 macros are used as l-value: ``Py_TYPE()``
and ``Py_SIZE()``.
...
be used as l-value to set these members.

The SWIG project use these macros as l-value to set these members. It
would be possible to modify SWIG to prevent setting directly
``PyDescrObject`` structure members, but it is not really worth it since
the ``PyDescrObject`` structure is not performance criticial and is
