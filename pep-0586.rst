...
-  Any: e.g. ``Literal[Any]``. ``Any`` is a type, and ``Literal[...]`` is
   meant to be values only. It is also unclear what ``Literal[Any]`` would
...
Literal types, enums, and exhaustiveness checks
-----------------------------------------------
...
checkers (such as mypy) do not yet implement this due to the predicted
complexity of the implementation work.

Some of this complexity will be alleviated once Literal types are introduced:
rather then entirely special-casing enums, we can instead treat them as being
approximately equal to the union of their values and take advantage of any
...
So here, the ``Status`` enum could be treated as being approximately equal
...
``Literal[False]``, and overloads to construct "custom type guards"::
   ...
   @overload
   def is_int_like(x: object) -> Literal[False]: ...
   def is_int_like(x): ...
