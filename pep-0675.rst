...
``LiteralString`` can be used as type arguments for generic classes:
...
+ ``StringLiteral``: Users might confuse this with the existing,
  unrelated concept of "string literals" in languages like `C++
  <https://en.cppreference.com/w/cpp/language/string_literal>`_.
...
compatible with ``LiteralString``. In other words, the ``capitalize``
method preserves the ``LiteralString`` type. There are several other
``str`` methods that preserve ``LiteralString``.
