...
Module-level comprehensions are not inlined
-------------------------------------------
...
optimizing their performance is low priority. Inlining them would require
separate code paths in the compiler to handle a module global namespace
dictionary instead of fast-locals, and it would be difficult or impossible to
avoid breaking semantics, since the comprehension iteration variable itself
would be a module global which might be referenced inside other functions
that are in turn called within the comprehension.

Some generator expressions may be inlined
-----------------------------------------

Generator expressions are a more complex case. A returned generator may escape
the function where the generator expression occurs, or may only be resumed
later in the function after other side effects may have occurred. In such cases
it must be a real resumable generator, which must be backed by its own
function, and can't be inlined.

There are some cases (code like ``tuple(x for x in lst)`` or ``any(x for x in
lst)``) where we can prove at compile time that the generator doesn't escape.
The compiler could identify and inline these, eliminating generator overhead.
The reference implementation does not yet implement inlining of any generator
expression (and it should be implemented and reviewed as a separate follow-up
pull request), but acceptance of this PEP also implies future inlining of
inlinable generator expressions.
