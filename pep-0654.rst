...
include in the new group. Since ``derive_new`` has access to self, it can
...
that of our new type.  Note also that our ``derive_new`` function does not
copy the ``__context__``, ``__cause__`` and ``__traceback__`` fields, because
``subgroup`` and ``split`` do that for us.
...
If we do not override ``derive_new``, then split calls the one defined
