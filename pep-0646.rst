...
This PEP would require two grammar changes. Full diffs of ``python.gram``
...
As with star-unpacking in other contexts, the star operator would call ``__iter__`` on the callee,
and add the contents of the resulting iterator to the list of index arguments.
This would enable ``TypeVarTuple`` to be implemented as roughly:
...
There are three things to note about this grammar change. First, for consistency with
...
::
    ...
    [*(1,)]      # = [1]
    [*(1,), 2]   # = [1, 2]
    C[int]       # __class_getitem__ receives x=int
    # But perhaps surprisingly:
    C[*Ts]       # __class_getitem__ receives x=(Ts._unpacked, )
    # This is consistent with:
    C[*Ts, int]  # __class_getitem__ receives x=(Ts._unpacked, int)


Second, the change would also enable star-unpacking of other structures within
indexing operations:
...
Third, more than one instance of a star-unpack can occur within an index:

::

    array[*idxs_to_select, *idxs_to_select]

The requirement that only one unpacked TypeVarTuple may be used per list of type parameters
would therefore be implemented only in type checking tools themselves rather than at the
syntax level.

Finally, note that star expressions within slices remain invalid:
...
The second change enables use of ``*args: *Ts``.
...
while matching the behaviour of a starred TypeVarTuple in an index: at runtime,
``__iter__`` is called on the TypeVarTuple, and a tuple containing the resulting
unpacked version of the TypeVarTuple is set as the type annotation for ``args``:

::

    >>> Ts = TypeVarTuple('Ts')
    >>> def foo(*args: *Ts): pass  # Equivalent to `*args: tuple(iter(Ts))`
    >>> foo.__annotations__
    {'args': (UnpackedTs,)}  # Where UnpackedTs is Ts._unpacked

Note that the only scenario in which this grammar change allows ``*Ts`` to be used as a direct
annotation (rather than being wrapped in e.g. ``Tuple[*Ts]``) is ``*args``. Other uses are still
invalid:
