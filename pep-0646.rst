...
``Array[Height, Width]``, we could also defined rank-3 arrays, rank-4 arrays,
and so on:
...
and is therefore only available in new versions of Python. To enable use of type
...
::
    ...
    def args_to_tuple(*args: Tuple[*Ts]) -> Tuple[*Ts]: ...
...
Also, for consistency, the type variable tuple must still be wrapped
within ``Tuple``:
...
(PEP 612 covers the use-cases where this might otherwise be useful.)
