...
stable C ABI (tagged as ``cp38-abi3-<platform>``). These same wheel
archives will also be usable on the subsequent 3.9 stable release series.
...
intended for library compatibility testing and the creation of ABI compatible
binary artifacts. It is not recommended for use in production environments.
