...
In recent years, that freeze date has been the last beta release, two
months before the expected final release date for X.Y.0. It isn't a
coincidence that this proposal places X.Y.0rc1 two months prior to
X.Y.0: it preserves that two month advance window for publication of
pre-built binary wheel archives.

With the proposed pre-freeze ABI flag in place, this aspect of the
release adoption process essentially continues on unchanged from the
status quo: a new CPython X.Y.0 series hits ABI freeze -> package
maintainers publish new binary extension modules for that stable
series.

The goal of the new pre-freeze ABI flag is then to improve the user
experience of the rolling pre-freeze releases themselve, by allowing
pre-built binary archives to be published for those releases without
any chance of users of the subsequent stable release inadvertently
