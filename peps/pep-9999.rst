...
For async generator expressions, which don't allow the internal use of
``YIELD_FROM``, we instead propose mimicking the functionality of the existing
``(z async for x in y for z in x)`` syntax more directly.  The resulting
bytecode for, for example, ``(*x for x in y)`` should be the same as the
bytecode for ``(z async for x in y for z in x)``, with the natural exception of
the ``STORE_FAST_LOAD_FAST`` used to bind the variable ``z``.
