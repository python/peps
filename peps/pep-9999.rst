...
without necessitating running a tool, once again to help with auditing.
...
- The array MAY not be exhaustive to allow for a smaller array as well as to
  ...
  same environment by having the array be mutually exclusive with other
  ``[[file-lock]]`` tables
- Lockers SHOULD sort the keys lexicographically to minimize changes when
  updating the file
...
- Aids in auditing by knowing that there are multiple entries for the same
  package that may need to be considered when auditing this specific package
