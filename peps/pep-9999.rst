...
2. :cls:`venv.EnvBuilder` gains a method:
   ``create_redirect_file(env_dir: os.PathLike, project_root: os.PathLike) -> None``.
   The method will create a redirect file at ``project_root / DEFAULT_NAME``
   that points to *env_dir*.
3. :meth:`venv.EnvBuilder.create` and :func:`venv.create` gain a keyword-only
   ...
   ``EnvBuilder.create_redirect_file()``. If the value for *env_dir* ends in
   ``DEFAULT_NAME`` and *project_root* points to the parent directory of
   *env_dir* then ``create_redirect_file()`` will not be called.
