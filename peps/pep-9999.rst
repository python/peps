...
* When generating metadata 1.2 or layer, producers **SHOULD** emit only
  ``Project-URL``, and **SHOULD NOT** emit ``Home-page`` or ``Download-URL``
  fields.
* When generating ``Project-URL`` equivalents for ``Home-page`` and
  ``Download-URL``, producers **SHALL** use the
  `label conventions <conventions-for-labels>` described below.
...
In plain language: a label is *canonicalized* by deleting all spans of dashes,
...
* Package indices: indices will continue to except well-formed core metadata,
...
* Outright removal of support for ``Home-page`` and ``Download-URL`` in the
  ...
  **SHALL** reject metadata containing these fields when said metadata is of
  the new major version.
* Enforcement of label canonicalization. If enforced, package producers
  **SHALL** emit only canonicalized ``Project-URL`` labels when generating
  distribution metadata, and package indices and consumers **SHALL** reject
  distributions containing non-canonicalized labels.

These potential changes would be backwards incompatible, hence their
inclusion only in this section.
