...
and their usage in :pep:`735`). The use of the ``.toml`` extension is to make
syntax highlighting in editors easier and to reenforce the fact that the file
format is meant to be human-readable.
...
- Optional
- Array of strings
- An unordered list of `wheel tags`_ which must be supported by the environment
- Lockers SHOULD sort the keys lexicographically to minimize changes when
  updating the file
