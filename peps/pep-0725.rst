...
The ``dep:`` URLs MUST be processed prior to their inclusion in the core
metadata by following these rules:
...
- If present, as per the
  ...
  the *version* field MUST be split from the URL and added next to it,
  between parentheses and after a space, with the following
  transformations:
