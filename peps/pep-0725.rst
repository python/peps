...
``vers:pypi/>=2.0``. This can be expressed as a two-item list:
``["pkg:pypi/numpy", "vers:pypi/>=2.0"]``. Additionally, a new proposal to add
the ``vers`` URL in a PURL qualifier is being `discussed
<https://github.com/package-url/purl-spec/pull/433>`__, but the current
direction requires percent-encoding and the repetition of some components for
disambiguation.
