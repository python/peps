...
``None``-aware access operators may only be used if the **complete**
primary expression is in a ``Load`` context. Assignments are not
permitted and will raise a ``SyntaxError``.
...
This does not apply if the "``None``-aware" primary expression can be
thought of as a self-contained part of a larger primary expression,
e.g. as part of an argument inside a call expression or an element
expression inside a container.
...
It is also possible to use them in **non-trivial**
`Parenthesized expressions <Parenthesized expressions - groupings_>`_
since in these the full "``None``-aware" primary expression is in a
``Load`` context. Though care must be taken so they can be evaluated
properly, e.g. with a fallback value.
...
::
    ...
    _t2 = _t1.b if ((_t1 := a) is not None) else None
    if _t2:
        _t2.c = 1
    else:
        d.c = 1
