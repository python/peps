...
+----------------------------------+---------------------------------------------------+
| ``.get(name)``                   | Get a "shareable" object from ``__main__``.       |
+----------------------------------+---------------------------------------------------+
...
(``Interpreter.bind()``), which sets some precedent for how objects
are shared.  More importantly, it facilitates the introduction of more
advanced approaches later and allows them to coexist and cooperate.
...
protocol (and C-API) used by ``Interpreter.bind()`` public.  With
that protocol, support for other types could be added
