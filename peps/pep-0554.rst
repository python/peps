...
interim solution, described in `API For Communication`_ below.
...
The type must implement the new protocol in `New C-API`_ below, which
is used to convert an object to interpreter-independent data and then
coverted back to an object on the other side.
Also see `is_shareable() <interpreters-is-shareable_>`_ above.

Only a minimal set of builtin types will be supported initially.
This will include ``None``, ``bytes``, ``str``, ``int``, and ``float``.
We may also support ``memoryview``, to allow sharing :pep:`3118` buffers.
...
previous versions of this PEP included such an approach: channels.
...
* ``PyErr_ChainExceptions()`` (AKA ``_PyErr_ChainExceptions1()``)
...
These all relate to tracking if an interpreter is currently running
