...
tasks will be whatever ``Interpreter.run()`` takes (e.g. a ``str``
script).  However, we may also support pickling the task and arguments,
to reduce friction (at the expense of performance for short-running
tasks).
...
It may be used to set up global variables before ``Interpreter.run()``
...
Clarify the role of ``Interpreter.run()``
-----------------------------------------

* rename the current ``Interpreter.run()`` to ``Interpreter.exec()``,
  preserving the restricted arg types?
* also keep ``Interpreter.run()`` (or add ``Interpreter.call()``)
  as a wrapper around ``Interpreter.exec()`` with a pickle fallback?

Address confusion about interpreters running in the current thread
------------------------------------------------------------------

One regular point of confusion is that ``Interpreter.run()`` executes
...
* by default, auto-run in a new thread?
* add ``Interpreter.run_in_thread()``?
* add ``Interpreter.run_in_current_thread()``?
* strictly rely on educating users?
