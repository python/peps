...
expressed in the current wheel format.
...
limitations`_. They include maintaining separate package indexes for
...
This problem is not unique to PyTorch. Projects like JAX_, NumPy_,
SciPy_, scikit-learn_ and many others in the scientific Python ecosystem
face similar hurdles. The core issue is that wheel tags are not
extensible enough to handle the combinatorial complexity of build
options. Wheel variants can capture GPU compatibility in wheel metadata
and select the correct wheel at install time.
