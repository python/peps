...
tends to choose to create a thread state for the main interpreter instead of
the current interpreter. This leads to thread-safety issues when extensions
create threads that interact with the Python interpreter, because assumptions
about the GIL are incorrect.
...
.. c:type:: PyInterpreterGuard
   ...
   will not finalize until the guard has been destroyed.
...
held. Any future finalizer that attempts to acquire the lock would be deadlocked.
