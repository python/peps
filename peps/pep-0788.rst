...
can acquire an interpreter guard at any point during its lifecycle, and
...
thread-safe way to acquire a guard for the main (or "default")
interpreter.
...
.. c:type:: PyInterpreterGuard
   ...
   until all threads have released the guard until it can enter finalization.

   This type is guaranteed to be pointer-sized.

.. c:function:: PyInterpreterGuard PyInterpreterGuard_FromCurrent(void)

    Acquire a guard for the current interpreter.
...
.. c:function:: PyInterpreterGuard PyInterpreterGuard_FromView(PyInterpreterView view)
    ...
    Acquire a guard to an interpreter through a view.
...
.. c:function:: void PyInterpreterGuard_Release(PyInterpreterGuard guard)

    Release an interpreter guard, possibly allowing the interpreter to shut
...
release the interpreter guard to allow the interpreter to shut down (and
...
.. code-block:: c
    ...
    thread_func(void *arg)
    {
        ...
        /* Release the interpreter guard, allowing it to
...
(``void *arg``), so it's difficult to acquire a guard to any specific
interpreter. The solution to this problem is to acquire a guard for the main
