...
While some data types support inherent default values, e.g. an empty
...
atomic to avoid race conditions where different threads try to use
...
``+=``, are not atomic either. In cases where race
conditions are an issue, existing synchronization primitives like locks
