...
``free-threading`` or ``gil-enabled``
    ...
    will be present, otherwise, the ``gil-enabled`` feature will be present.
    The two are mutually exclusive. On Unix systems, the presence of
    ``free-threading`` corresponds to ``"t" in sys.abiflags``.
...
``32-bit`` or ``64-bit``
    ...
    build [#bitness]_. The two features are mutually exclusive.
