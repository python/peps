...
objects through ``pickle``.  Otherwise it is the same as ``SharedQueue``.

The difference, support for non-shareable objects, means objects
popped off the queue are always copies.  Thus, when passed through an
``interpreters.Queue``, changes to a mutable object in one interpreter
do not apply to the corresponding object in another interpreter.
