...
``SharedQueue`` class for communication between interpreters.
...
There are ``interpreters.SharedQueue`` objects, but they only proxy
...
between interpreters may be sent through a ``SharedQueue``.
...
* ``create_shared_queue(maxsize=0) -> SharedQueue``
...
``interpreters.SharedQueue`` objects act as proxies for the underlying
cross-interpreter-safe queues exposed by the ``interpreters`` module.
Each ``SharedQueue`` object represents the queue with the corresponding
unique ID.
There will only be one object for any given queue.

``SharedQueue`` implements all the methods of ``queue.Queue`` except for
...
``interpreters.Queue`` is a subclass of ``interpreters.SharedQueue``
that isn't limited to just shareable objects.  It supports the extra
objects through ``pickle``.  This applies specifically to the
``put()`` and ``put_nowait()`` methods, and indirectly to the
``get()`` and ``get_nowait()`` methods.  Other than that,
it is the same as ``SharedQueue``.

The difference, support for non-shareable objects, means objects
popped off the queue are always copies.  Thus, for a mutable object
passed through an ``interpreters.Queue``, changes to the object
in one interpreter do not apply to the corresponding object
in another interpreter.

Shareable Objects
-----------------

Both ``Interpreter.prepare_main()`` and ``SharedQueue`` work only with
"shareable" objects.
...
Most objects will be shared through queues (``interpreters.SharedQueue``),
...
``Interpreters.SharedQueue`` and ``memoryview``.  Mutating the object
...
synchronization primitives.  Instead, ``interpreters.SharedQueue``
...
pass an object around to indicate who can use the resource::
   ...
   numworkers = 10
   control = interpreters.create_shared_queue()
...
still be done through ``interpreters.SharedQueue`` objects,
...
::
   ...
   tasks = interpreters.create_shared_queue()
   results = interpreters.create_shared_queue()
...
create(), create_queue(), and create_shared_queue()
---------------------------------------------------
...
to get a new interpreter or ``create_shared_queue()`` for a new queue.
