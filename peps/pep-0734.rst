...
* ``call(callable, /)``
      ...
      then it gets propagated in the same way as ``Interpreter.exec()``,
      except with ``CallFailure`` instead of ``ExecFailure``.
...
* ``create_queue(maxsize=0, *, sharedonly=False) -> Queue``
      Create a new queue.  If the maxsize is zero or negative then the
      queue is unbounded.

      "sharedonly" is used as the default for ``put()`` and ``put_nowait()``.
...
* ``put(obj, timeout=None, *, sharedonly=None)``
      ...
      If "sharedonly" is None (the default) then the queue's default
      value is used.

* ``put_nowait(obj, *, sharedonly=None)``
