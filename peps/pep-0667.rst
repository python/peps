...
:pep:`558` kept an internal copy of the local variables in an
effort to improve backwards compatibility with the legacy
``PyEval_GetLocals()`` API, whereas this PEP does not.

:pep:`558` did not specify exactly when the internal copy was
updated, making the behavior of :pep:`558` impossible to
reason about in some cases where this PEP remains well
specified.
