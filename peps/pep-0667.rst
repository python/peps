...
execution APIs that default to executing code in the ``locals()`` namespace.
...
of a new write-through proxy type that directly modifies the optimized
...
Removing keys with ``del``, ``pop()`` or ``clear()`` is NOT supported.
...
Accordingly, the function will be marked as soft-deprecated in Python 3.13, and subsequently
hard-deprecated in Python 3.14, with a target removal date of Python 3.16 (two releases
after Python 3.14). Alternatives wil be recommended as described below.
