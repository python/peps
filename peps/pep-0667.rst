...
``locals()`` is implicitly used as a scope shared between multiple ``exec()``s.
...
With the semantic changes to ``locals()``, it's easier to explain the behavior
...
However, you can still achieve the wanted behavior with explicit scopes::
...
``frame.f_locals`` which is not possible before::
