...
Exclusive Reliance on Digital Attestations
------------------------------------------

The idea [5]_ here would be to solely rely on :pep:`740` attestations to verify
certain properties of dependencies, such as:

* The package was uploaded by a specific organization or user name
* The package was uploaded by an owner of a specific domain name
* The package was uploaded by a user with a specific email address

A general downside to this approach is that implementing this on custom package
repositories would require a significant amount of work. In the case of PyPI,
the prerequisite work on `Trusted Publishing`__ and then the
`PEP 740 implementation`__ itself took the equivalent of a full-time engineer
one year whose time was paid for by a corporate sponsor. This is unlikely to be
implemented by enterprise because it's possible to implement reproducible
builds already using simpler mechanisms, and such attestations are not very
useful when everything is internally managed.
...
The amount of effort wouldn't matter for community projects because they
wouldn't be able to maintain any infrastructure themselves in most cases.

Another weakness is that a user attempting to perform a simple ``pip install``
would already be vulnerable to malicious packages. In order to overcome this,
there would have to be the concept of implicit constraints, all of which would
impose certain UX or resolver logic upon every tool.
...
the attestation details. This would be very confusing and noisy, especially for
new users, and would be a breaking UX change for existing users. Many methods
of installation wouldn't work for this scenario such as installing from a
requirements file where the user would potentially be getting hundreds of
prompts.

One solution to make this less disruptive for the user would be to manually
...
`entry points`__, which corporate environments could install by default. This
has the major downside of not providing automatic guarantees which would limit
the usefulness for the average user who is more likely to be affected.

__ https://packaging.python.org/en/latest/specifications/entry-points/

An idea to provide automatic protection would be to have a set of default
packages that would be installed by default. Each repository could offer a
service that verifies the owner of a package so as to reduce the risk of
malware distribution. After verification, the repository would add the
details to a dedicated package that would be installed by default. This would
require dedicated maintenance which is unrealistic for most repositories, even
PyPI currently. It's unclear how community projects without the resources for
something like a domain name would be supported. Critically, this solution
would cause extra confusion for users in the case of multiple repositories as
each might have their own verification processes, attestation criteria and
default package containing the verified details.

Although the exclusive reliance on attestations is not viewed as a viable
alternative, their use should be encouraged as they do offer interesting
security guarantees. For example, a tool could offer the ability to verify
packages with an associated namespace come from authorized parties (as
discussed in the `Security Implications <security-implications_>`_ section).

Attesting Package Owner Names
-----------------------------
...
repository and could be implemented by exposing extra metadata or as a
`digital attestation <digital-attestations_>`_.
...
.. [5] `Detailed write-up <https://discuss.python.org/t/64679>`__ of the
   potential applications of digital attestations.
