...
level (often at the top!). This avoids repetition, makes dependencies clear
...
It obfuscates the full set of dependencies of a module. Additionally, there's a
slight penalty to each invocation of the function, as the import and binding must
be re-resolved.
...
These approaches don't cover all cases, add runtime overhead in unexpected places and
are non-obvious and non-standardized. This proposal introduces lazy imports with a
design that is local, explicit, controlled, and granular. Each of these qualities
is essential to making the feature predictable and safe to use in practice.
...
* Check if the module already exists in ``sys.modules``, and return it if so.
