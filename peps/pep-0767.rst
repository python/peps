...
and is useful for removing the ability to mutate an attribute at a type checker level,
...
* The ``HasName`` protocol has a more succinct definition,
  and can be implemented with custom descriptors or class variables.
...
Type checkers should error on any attempt to mutate an attribute annotated with ``ReadOnly``.
...
* In ``@classmethod``\ s, on instances of the declaring class created via a call
  to a super-class' ``__new__`` method.
