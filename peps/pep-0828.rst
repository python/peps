...
or asynchronous, that is used inside of a ``yield from``.
...
significantly different bytecode for ``yield`` statements inside of asynchronous
...
author of this PEP is not completely convinced for one case or the other.
...
generator would delegate to a synchronous generator from an asynchronous one
(and ``async yield from`` was the counterpart for asynchronous subgenerator
