...
- Supporting multiple ``TypedDict`` unpacks (e.g.
  ``Callable[[Unpack[TD1], Unpack[TD2]], R]``) was considered. This would
  introduce a concept with no equivalent in the callback Protocol approach on
  how these dicts are merged and how conflicts are resolved.
  It would introduce a new concept — how multiple keyword                                                              
  argument sets are merged and how key conflicts are resolved — that has no                                                             
  precedent in the existing typing system and would require significant                                                                 
  further design work. Consensus was to not introduce it.
  ``TypedDict`` inheritance can combine multiple TypedDicts when needed.
