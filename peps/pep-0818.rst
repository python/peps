...
We introduce two new top level packages, ``js`` and ``pyodide``. The ``js``
package is the JavaScript global scope ``globalThis``. What set of values are
present on the ``js`` module depends on the JavaScript runtime and whether the
Python runtime is in the main thread or a worker thread. For instance
``from js import Buffer`` will succeed in Node but not in a browser.

The ``pyodide`` package has two modules: ``pyodide.code`` and ``pyodide.ffi``.
``pyodide.code`` exposes the ``run_js`` function. ``pyodide.ffi`` exposes the
following functions:
