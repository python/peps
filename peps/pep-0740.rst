...
* When an uploaded file has one or more attestations, the index **MAY**
  ...
  For example, if an uploaded file is hosted at the URL
  ``https://example.com/sampleproject-1.2.3.tar.gz``, the provenance URL would
  be ``https://example.com/sampleproject-1.2.3.tar.gz.provenance``.

* When a ``.provenance`` file is present, it **SHALL** contain
  a JSON-encoded :ref:`provenance object <provenance-object>`.
...
* When the ``data-provenance`` attribute is present, its value **SHALL** be
  the SHA256 digest of the ``.provenance`` file's contents.
