...
.. code-block:: python
  ...
  class Provenance:
      ...
      attestations: list[tuple[Publisher, list[Attestation]]]
      """
      One or more attestation objects, indexed by their publisher.
      """

* ``version`` is ``1``. Like attestation objects, provenance objects are
  versioned, and this PEP only defines version ``1``.
* ``attestations`` is a **required** JSON array, containing pairs
  of publishers and a JSON array of one or more attestation objects.

  Each publisher is a Trusted Publishing identity, as described
  by the ``Publisher`` model above. As noted in the ``Publisher`` model,
  each publisher object is specific to its Trusted Publisher but must include
  at minimum:
