...
To make usage of sentinels convenient in typed Python code, we propose to add
amend the type system with a special case for sentinel objects.

Sentinel objects may be used in :typing:term:`type expressions <type expression>`,
representing themselves.
This is similar to how ``None`` is handled in the existing type system.
For example::
...
``NAME = Sentinel('NAME')`` as creating a new sentinel objects. If the name
...
:typing:term:`type expressions <type expression>`. They
represent a :typing:term:`fully static type` that has a single member, the sentinel object itself.
