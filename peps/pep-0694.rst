...
#. Initiate an :ref:`Upload session <session-create>`, creating a release stage.
#. Initiate :ref:`File Upload Session(s) <file-upload-session>` to that stage
   as part of the upload session.
...
If the server is able to immediately complete the Upload Session, it may do so and return a
``201 Created`` response. If it is unable to immediately complete the Upload Session
...
In either case, the server should include a ``Location`` header pointing back to the Upload Session
...
To cancel an Upload Session, a client issues a ``DELETE`` request to the ``session`` :ref:`link
...
When creating an Upload Session, clients can provide a ``nonce`` in the
:ref:`initial session creation request <session-create>`.
