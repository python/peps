...
* an publishing session, which can be used to simultaneously publish
...
``nonce`` (**optional**)
    An additional client-side string input to the :ref:`"Publishing Session Token" <publishing-session-token>`
...
``links``
    A dictionary mapping :ref:`keys to URLs <publishing-session-links>` related to this session, the details of
...
``session-token``
    ...
    the unique :ref:`"session token" <publishing-session-token>` that can be provided to installers in order to
...
``session``
    ...
    session <publishing-session-completion>`, :ref:`canceling and discarding the session <publishing-session-cancellation>`,
...
``link``
    ...
    to retrieve, replace, or delete the :ref:`referenced file <file-upload-session>`.  If a ``nonce`` was
...
``POST`` request to the ``session`` :ref:`link <publishing-session-links>` given in the :ref:`session creation
...
In either case, the server should include a ``Location`` header pointing back to the Publishing Session
status URL, and if the server returned a ``202 Accepted``, the client may poll that URL to
...
<publishing-session-links>` given in the :ref:`session creation response body <publishing-session-response>`.  The server
then marks the session as canceled, and **SHOULD** purge any data that was uploaded as part of that
session.  Future attempts to access that session URL or any of the Publishing Session URLs **MUST**
...
<publishing-session-create>`, then the session token is easily guessable from the package name and version
number alone.  Clients can elect to omit the ``nonce`` (or set it to the empty string themselves) if
they want to allow previewing from anybody without access to the session token.  By providing a
...
<file-upload-session>` sequence over again.  This means providing the metadata request again to
retrieve a new upload resource URL.
...
``publishing-session`` :ref:`link <publishing-session-links>` or ``file-upload-session`` :ref:`link <file-upload-session-links>`
...
The server will respond to this ``GET`` request with the same :ref:`Publishing Session creation response body <publishing-session-response>`
or :ref:`File Upload Session creation response body <file-upload-session-response>`,
...
will respond with the same :ref:`Publishing Session creation response body <publishing-session-response>`
or :ref:`File Upload Session creation response body <file-upload-session-response>`,
that they got when they initially created the Publishing Session or File Upload Session,
except with any changes to ``status``, ``valid-for``, or ``files`` reflected.
...
sub-key of the :ref:`links key <publishing-session-links>` returned when the Publishing Session is created.
...
<publishing-session-completion>` without uploading any files.  While the ``version`` key is required in the JSON
