...
`Eric Snow proposed
<https://discuss.python.org/t/pep-741-python-configuration-c-api-second-version/45403/27>`_
to have multiple "phases" for the initalization, not only the
"preinitialization" and the "initialization". One advantage is to be
able to configure Python using Python code rather than C code for last
phases. See :pep:`432` which proposed a similar concept of multiple
phases, whereas :pep:`587` was implemented only with the
preinitialization and the initialization.

Conceptually, the runtime can be divided into 5 distinct parts and
initialization/config may be split along those lines:
...
Under the status quo, config/initialization/finalization is currently
all tangled together. That translates into a consistent maintenance
cost. The situation is largely a consequence of organic growth over the
decades (see :pep:`432`), but has gotten better since around 2017.

Eric's ideal equivalent would be something like this:

1. User initializes **host process resources**.
2. User populates a **configuration**:
...
3. Initialize the runtime
   ...
   * ``Py_InitializeFromConfig()``: ``PyConfig`` → **global runtime state**.

4. Create/Initialize the main interpreter

   * ``Py_NewInterpreterFromConfig()`` creates a new interpreter:
...
7. User starts using the C-API: state → behavior.
...
* Populate configs: ``PyMain_InitConfig()`` or something like that.
