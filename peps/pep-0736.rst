...
A common IDE feature is to highlight other references in the code to the same
value as that at the current caret position. With this shorthand syntax, when
the caret is on the argument name it may be valuable to highlight both
references to the argument and its value reflecting the fact that this name now
refers to both.
