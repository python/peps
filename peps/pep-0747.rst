...
is a ``TypeExpr``. By contract ``isinstance`` only accepts a simple class object
(a ``type[]``) as input:
...
If you *do* want to write your own runtime type checker or some other
kind of function that manipulates types as values at runtime, please see
:ref:`advanced_examples`:
