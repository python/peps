...
The type expression ``TypeForm``, with no type argument provided, signals that
the expression assigned to this annotated type should be interpreted as a type
expression, resulting in a ``TypeForm`` type, but the type parameter of that
``TypeForm`` type should be inferred from the type expression. For example,
these two assignments are equivalent::

  tf1: TypeForm = int | None
  tf2: TypeForm[int | None] = int | None
