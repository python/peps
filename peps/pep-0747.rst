...
A ``TypeExpr``, as a consequence of being a type expression,
may be a string literal that spells a forward reference:
...
However runtime type checkers already must still be prepared to recognize and
...
parameterized by a type variable that is also used by a ``TypeIs[]``
...
However ``TypeExpr[]`` already treats 
:ref:`Annotated[T, metadata] as equivalent to T anyway <interactions_with_annotated>`.
