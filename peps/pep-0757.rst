...
.. c:function:: PyLongExport_Kind PyLong_Export(PyObject *obj, PyLongExport *export)
   ...
   given value, see :c:struct:`PyLongExport`.  :c:func:`PyLong_FreeExport` must
   be called when the export is no longer needed.  Currently the only available
   type is ``PyLongExport_DigitArray``.
