...
* Having package installers recognize a host-relocatable venv and generate
  appropriate e.g. script wrappers.
...
expected behavior. Thus, this requirement should be lifted.

Second, virtual environments are the standard way for running Python
applications, which means how they're created is important. This means
recreating a virtual environment for each instance ...

When they need to be recreated ...

Currently, virtual environments are effectively tied to the particular host
that the virtual environment is originally created on. This is because symlinks
to absolute paths are commonly used in various places (e.g. `bin/python3`),
which are frequently specific to a host (e.g. by containing a user's home
directory).
