...
The lack of a standard for ``requirements.txt`` contents also means that not
portable to any alternative tools which wish to process them other than
``pip``.
...
does not build a distribution (is not a package).
...
``requirements.txt`` files for ``pip``, support several nonstandard extensions
...
PDM restricts itself to a string syntax, and Poetry introduces objects which
