...
Use of ``[project.optional-dependencies]`` outside of packages
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

Although some tools (e.g., ``uv``) define direct use of the
``[project.optional-dependencies]`` table, this is distinct from the usage of
this table defined in :pep:`621`, and should be better understood as a direct
read of TOML data from ``pyproject.toml``.

Under :pep:`621`, use of the ``[project]`` table stipulates required fields
(``name`` and ``version``). Therefore, using
``[project.optional-dependencies]`` without specifying these fields means that
a project is not defining :pep:`621` metadata. This may or may not be allowed,
and in particular many tools will treat partial package metadata as an error.
