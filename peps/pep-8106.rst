PEP: 8106
Title: AnnotatedMeta for type checking of PEP 593 Annotated
...
This PEP adds ``AnnotatedMeta`` to the typing module, which is a generic class
that when unsubstituted is equivalent to the type in ``Annotated``.

For example::
    from typing import AnnotatedMeta

    class Int64(AnnotatedMeta[int]):
        ...
  
    Annotated[int, Int64()]  # okay
    Annotated[float, Int64()]  # error because AnnotatedMeta[float] != AnnotatedMeta[int]
...
converter that uses ``AnnotatedMeta`` and we can agree upon a way to express allowing an input
...
The following class is added to the typing module::
    ...
    class AnnotatedMeta[FieldT]:
        ...

Libraries and tools that want to support ``Annotated`` type checking have their
metadata inherit from it::

    from typing import AnnotatedMeta, Any, Callable, overload

    class Int64(AnnotatedMeta[int]):
        ...

    class Validate[In: Any, Out](AnnotatedMeta[Out]):
        @overload
...
follow the behavior that if ``AnnotatedMeta`` is un-parameterized it's type parameter
should take the value of the type in ``Annotated``.
As of writing `pyright <pyright>`_ is the only type checker that supports type checking metadata in ``Annotated``.

Type checkers that do not type check metadata in ``Annotated`` will continue to ignore
this change and are not impacted by it.
