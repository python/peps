...
it is useful to be able to check that the metadata makes sense for the given type.
...
* The metadata element evaluates to an object ``M`` that has a ``__supports_type__`` method, and
  ...
  parameter type of the ``__supports_type__`` method) and does not evaluate to ``Literal[False]``.

The body of the ``__supports_type__`` method is not used to check the validity of the metadata
and static type checkers can ignore it, but tools that use the annotation at runtime may call the
method to check that a particular value is valid.
...
and end users of those libraries are unlikely to need to implement the protocol themselves.
