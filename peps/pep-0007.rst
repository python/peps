...
* Python 3.12 provided best effort support for the
  ``-Werror=declaration-after-statement`` compiler flag (forbid mixed
  declarations and code) for the C API. It is no longer the case in
  Python 3.13: this flag should not be used with the Python C API.
