...
It was suggested to reuse ``Literal`` from ``typing.Literal`` to match
simple names. So far no names other than ``_`` have a dedicated meaning
in the match statement mini-syntax. Assigning one for ``Literal`` would
thus be surprising. Additionally, subscription isn't defined for use in
match cases yet. Using it here would limit future options unnecessarily.
Lastly, ``typing.Literal`` only supports literal values as arguments,
