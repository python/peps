...
Discussions-To: https://discuss.python.org/t/pep-845-leading-dot-value-patterns/109100
Status: Draft
Type: Standards Track
Created: 25-Aug-2026
Python-Version: 3.16
Post-History: `17-Sep-2026 <https://discuss.python.org/t/pep-845-leading-dot-value-patterns/109100>`__


Abstract
========

This PEP enables the use of non-attribute names as **value patterns** in match
...
in particular those defined in the same module. While other workarounds exist to
...
The value pattern will be extended to support simple names
...
https://github.com/cdce8p/cpython/tree/pep845-match-leading-dot.
An online demo can be tested at https://pep845-demo.pages.dev.
...
It was suggested to reuse ``Literal`` from :py:class:`typing.Literal` to
match simple names. So far no names other than ``_`` have a dedicated meaning
in the match statement mini-syntax. Assigning one to ``Literal`` would
thus be surprising. Additionally, subscription is not defined for use in
match cases yet. Using it here would limit future options unnecessarily.
Lastly, :py:class:`typing.Literal` only supports literal values as arguments,
e.g. ``1``, ``"Hello"`` or ``True``. Allowing simple names only inside
match cases would therefore be confusing.
