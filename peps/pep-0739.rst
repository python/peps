PEP: 739
Title: ``build-details.json`` 1.0 — a static description file for build details of Python installations
...
.. list-table::
    ...
    * - Description
      - Valid module suffixes grouped by type.
        ...
        However, if the Python implementation does not provide suffixes
        of the kind specified by any of the attributes, the equivalent
        sub-section is not required to be present. Additionally, if a
        Python implementation provides extension kinds other than the
        ones listed on ``importlib.machinery`` module, they **MAY** add
        a sub-section for them.
    * - Examples
...
.. list-table::
    ...
    * - Description
      - The path to the pkg-config definition files.
        ...
        This field **MUST** be present if the Python implementation
        provides pkg-config definition files for the C API, otherwise
