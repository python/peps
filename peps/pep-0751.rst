...
- Lockers SHOULD record packages in order by ``packages.name``
  lexicographically, ``packages.version``.
...
- Optional
- Array of strings
- A list of names representing a named group of entries in ``[[packages]]``.
- Acts as a way to designate and name a subset of the dependency graph.
...
``packages.dependencis.extras``
'''''''''''''''''''''''''''''''

- Optional
- Array of strings
- The names of the extras specified for the dependency requirement
...
- Optional
- String
- The `version specifiers`_ for the dependency requirement.
...
- Optional
- String
- The `environment marker`_ for the dependency requirement.
...
- Required if ``path`` is not set
- String
- The URL to the source distribution file.
...
- Required if ``url`` is not set
- String
- A path to the source distribution, which may be absolute or relative.
- If the path is relative it MUST be relative to the lock file.
...
- Lockers MUST make sure the tag value is unique within ``packages.wheels``.
...
- Required if ``url`` is not set
- String
- A path to the wheel file, which may be absolute or relative.
- If the path is relative it MUST be relative to the lock file.
