...
XXX what do when trying to maximize versions for 2 different extras is ambiguous? E.g., A>=2 and A<4 w/ A==5
...
def install(lock_file_data, *selected_groups):
    """Install the selected groups from the lock file.
    ...
    """
    available_packages = []  # What values from '[[packages]]' can be considered.
    requirements_by_package = {}  # The list of requirements for a package.

    # Treat a package by name as a node in the graph, gathering all edges/requirements
    # pointing to the package.
    for group in selected_groups:
        gather_requirements(locked_file_data["packages"], group,
                            requirements_by_package, available_packages)

    # With all the requirements gathered per package, we can now check whether there is
    # any ambiguity with the available package versions to choose from  across groups.
    package_version_to_install = []
    package_clusters = cluster_by_name(locked_packages)
    for package_name, requirements in requirements_by_package.items():
        potential_versions = filter(lambda x: x["name"] == package_name, available_packages)
        for req in requirements:
            potential_versions = filter_versions(req, potential_versions)
        version_options = list(potential_versions)
        if not version_options:
            raise UnsatisfiableError(f"no package version for {package!r} found that satisfies {requirements!r}")
        elif len(version_options) > 1:
            raise AmbiguityError(f"multiple package versions for {package!r} found that satisfy {requirements!r}")
        else:
            package_version_to_install.append(version_options[0])

    # Figure out the artifacts we want to install, raising an exception if no suitable
    # artifact is available for the environment (e.g., no wheels).
    to_install = map(find_artifact, package_version_to_install)
...
def gather_requirements(locked_packages, group, requirements_by_package, all_available_packages):
    """Collect all requirements for a group.

    'requirements_by_package' is added/extended with any requirements found.
    'all_available_packages' is extended with any package versions flagged as part of the
    group.
    """
    group_name, extras = group

    # Gather all of the package versions for this group.
    available_packages = {}
    for package in locked_packages:
        if group_name not in package["groups"]:
            continue
        available_packages.setdefault(package["name"], []).append(package)

    # Walk the graph, gathering the relevant packages and their requirements.
    ...
    # XXX


def cluster_by_name(locked_packages):
    ...


def filter_versions(requirement, potential_versions):
    ...


def find_artifact(package_version):
    ...
