...
The closest the community has to a standard are pip's ``requirements.txt`` files
which all the aforementioned tools either use directly as their file format or
export to. Unfortunately the format is not a standard but is supported by
convention. It's also designed very much for pip's needs, limiting its
flexibility and ease of use (e.g., it's a bespoke file format). Lastly, it is
not secure by default (e.g., file hash support is entirely an opt-in feature,
you have to tell pip to not look for external dependencies instead of the file
encoding such details, etc.).
...
of ``requirements.txt`` files that are used as a lock file (e.g., what
