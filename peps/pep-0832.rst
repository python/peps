...
This PEP sets out to help make the discovery of a project's preferred virtual
environment easier. By providing a default location to look for a virtual
...
the default location, tools will have a way to find the project's preferred
virtual environment.
...
no standardized way to tell anyone where the preferred virtual environment is?
...
the (current) virtual environment to be used for the project is located
(which, just like a :file:`.venv` directory, can be a logical or pysical path).
This virtual environment may not be the only environment associated with the
...
Tools looking for a virtual environment SHOULD look for either a :file:`.venv`
directory or a :file:`.venv` redirect file. IF a tool chooses to search parent
directories, THEN the closest :file:`.venv` in either form SHOULD be selected.
...
containing a virtual environment when being written. The path MAY use POSIX
path separators-- ``/`` --regardless of what the operating system's native path
separator is. The path MAY be relative, and if so MUST be relative to the
directory containing the :file:`.venv` file.
...
environment when no previous environment selection has occurred or the tool
doesn't have its own justification for ignoring the user's supposed choice.
...
even use the :file:`.venv` name. In those situations, you need *some* way
...
Tools need to treat the contents of a :file:`.venv` redirect file as path data
...
3. Existing tools seem to already support :file:`.venv`
4. One can still use a different name for an environment thanks to
   :file:`.venv` redirect files as proposed by this PEP
...
It was suggested to have :file:`.venv` redirect files record what tool provided
...
and not worth complicating :file:`.venv` redirect files for. If a tool wanted
...
Using a more structured data format such as JSON for :file:`.venv` redirect
files was suggested. Typically it was in order to record details about the
environment directly in :file:`.venv`. But since that would be redundant data
...
:file:`.venv` redirect file.


Leave .venv redirect files out of the PEP
-----------------------------------------

Some have suggested leaving :file:`.venv` redirect files out of the PEP (or not
...
seemed strong enough to keep :file:`.venv` redirect files included.
...
There was a suggestion to allow for multiple :file:`.venv` redirect files,
...
This PEP was first published supporting :file:`.venv` redirect files, but then
some community pushback led to switching to :file:`.python-envs` files which
...
the preferred environment was unexpectedly, etc. And with :file:`.python-envs`
...
The :file:`.python-envs` proposal mentioned above was also going to allow
...
`vote on communication protocols <https://discuss.python.org/t/106998/117>`__
and `continue that discussion <https://discuss.python.org/t/108033>`__.
...
- 24-Sep-2026
  ...
  - Clarify :file:`.venv` redirect files can be logical or physical paths just
    like :file:`.venv` directories
  - Specify that tool can ignore supposed user selections if the tool has a
    justification to
  - Clarify that :file:`.venv` redirect files have to be valid paths
    **when written**; this can't really be enforced upon reading

- 08-Sep-2026

  - Switch back to :file:`.venv` redirect files from :file:`.python-envs` files
