...
having this PEP at all). But during discussions around this PEP, the desire to
have a way to list the location of multiple environments no matter where they
live seemed strong enough to keep :file:`.venv` redirect files included.
...
allowed for listing multiple virtual environments. The thinking was enough
projects have multiple virtual environments that listing all of them would be
useful.

But then there was more tool pushback or indifference to :file:`.python-envs`.
In the end it was decided to go back to :file:`.venv` redirect files as there
was not enough motivation to support multiple environments for a simpler,
