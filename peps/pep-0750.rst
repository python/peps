...
has a short section on implementing `structured logging <https://docs.python.org/3/howto/logging-cookbook.htmlhttps://docs.python.org/3/howto/logging-cookbook.html#implementing-structured-logging>`_.

The logging cookbook suggests creating a new "message" class, ``StructuredMessage``,
that is constructed with a percent-style format string and a dictionary of values.
The class's ``__str__()`` method formats both the human-readable message and the v
alues as a JSON string, combining them both into a final string. (See the
...
Use of template strings gives us a more elegant way to define the custom message
class. Unlike with the logging cookbook's example, it is not necessary for
developers to make sure that their format string and values dictionaries are
kept in sync. The ``TemplateMessage`` class can automatically extract the values
and their keys from the template string's ``Interpolation.value`` and 
``Interpolation.expr`` attributes, respectively.
...
**TODO** Dave. The code is written, and I really like this approach, so I 
plan to write just a few more words here about the example code.

Example: HTML Templating
-------------------------

This PEP has several examples of using t-strings for HTML templating. It turns
out that the "hypotheical" ``html()`` function mentioned in the `Motivation`_
section (and a few other places in this PEP) exists and is available in the
