...
scoping semantics for names used in class definitions, so as to support a
somewhat similar deferred execution model for annotations. Of course tag string
usage of this scoping is otherwise **not at all** about annotations. So this
PEP, and the corresponding reference implementation, only uses annotation scope,
and not the other machinery specifically supporting annotations.
