...
common libraries will provide support for callable or awaitable ``value``\ s in 
...
.. code-block:: python
    ...
    assert await aprocess(template) == "Hello World"

This assumes that the template processing code is asynchronous and is able to
await the result of the interpolation. 
