...
The use of ``@dataclass`` in the definition of ``Template`` is meant to be 
suggestive; the exact implementation in ``cpython`` may differ, but developers 
can expect that ``Template`` instances can be constructed with a single 
``args`` attribute that is a sequence of strings and ``Interpolation`` instances.
...
for detailed examples of how to use template strings.
...
Structured logging allows to log data in both a structured format, such as JSON,
as *well* as a human-readable format, at the same time. This is useful for log 
aggregation systems that can parse the structured format, but also for 
developers who want to read the logs directly.

We present two different approaches to implementing structured logging using
template strings.

Approach 1: Custom Log Messages
'''''''''''''''''''''''''''''''

The `Python Logging Cookbook <https://docs.python.org/3/howto/logging-cookbook.html>`_ has a brief section
on implementing `structured logging <https://docs.python.org/3/howto/logging-cookbook.htmlhttps://docs.python.org/3/howto/logging-cookbook.html#implementing-structured-logging>`_.

The logging cookbook introduces a simple new class, ``StructuredMessage``, that
takes a percent-style format string and a dictionary of values. We repeat the
key code and example here:
...
.. code-block:: python
    ...
    # Outputs "message 1 >>> {"foo": "bar", "bar": "baz", "num": 123, "fnum": 123.456}"
...
.. code-block:: python
    ...
    foo, bar, num, fnum = "bar", "baz", 123, 123.456
    logging.info(_(t"The {foo} is {bar} and {num} is close to {fnum:.2f}"))

    # Outputs (without indentation):
    # {
    #    "message": "The bar is baz and 123 is close to 123.46"
    #    "values": {"foo": "bar", "bar": "baz", "num": 123, "fnum": 123.456}
    # }

Use of template strings gives us a more structured way to define the message
