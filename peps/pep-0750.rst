...
It is easy to "implement" f-strings using t-strings. Specifically, we can 
write a function `f(template: Template) -> str` that processes a template string
in much the same way as an f-string, returning the same result:
