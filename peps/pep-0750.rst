...
an ealier version of this PEP allowed for arbitrary "tag function" prefixes in front
of literal strings:
...
- It was deemed too complex to build in full generality. JavaScript allows for 
  arbitrary expressions in the tag function which is a significant challenge to 
  implement in Python.
- It seemed to needlessly pollute the namespace with new syntax.
...
An early version of this PEP proposed that interpolations be lazily evaluated. All
interpolations were "wrapped" in implicit lambdas. Instead of having an eagerly
evaluated ``value`` attribute, interpolations had a ``getvalue()`` method that would
...
- There are many use cases where eager evaluation is more convenient and efficient;
  it is the clear default.
- Implicit lazy evaluation approach leads to difficulties with type hints and
  static analysis.

There are also viable alternatives to lazy evaluation. 
...
If developers wish to reuse template strings multiple times with different values,
they can write a function to return the ``Template`` instance:

.. code-block:: python

    def hello_template(name: str) -> Template:
        return t"Hello {name}"

    assert hello_template("World").args[1].value == "World"
