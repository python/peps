...
avoid an `SQL injection attack <https://en.wikipedia.org/wiki/SQL_injection>`__.
...
string processing tasks. Template strings address these problems by providing 
...
.. code-block:: python
    ...
    attributes = {"class": "joy", "id": "main"}
    template = t"<div {attributes} />"
    assert html(template) == '<div class="joy" id="main" />'

By providing a way to intercept and transform interpolated values, template strings
enable a wide range of use cases, from injection prevention to complex DSLs.
...
``t`` and uppercase ``T`` prefixes are supported. 
...
The ``format_spec`` attribute is the `format_spec string <https://docs.python.org/3/library/string.html#format-specification-mini-language>`_.
As with f-strings, this is an arbitrary string that can be used to format the value:

.. code-block:: python

    name = "World"
    template = t"Hello {name:.2f}"
...
be surprising if, for example, a template string that uses ``{name:.2f}`` did not
...
.. code-block:: python
    ...
    def lower_upper(template: Template) -> str:
        ...
        parts = [
            arg.lower() if isinstance(arg, str) else arg.value.upper()
...
There is no requirement that template strings be processed in any particular way;
there's no special machinery that processes them. It's worth mentoning that
code that processes templates has no obligation to return a string. Template 
strings are a general-purpose and flexible facility.
...
support explicit concatenation using ``+``. Concatenation may have different
semantics in different DSLs or no semantics at all; accordingly, the 
``Template`` type that t-strings evaluate to does not support ``__add__`` 
or similar methods.
...
In particular, ``t'{expr=}'`` is converted into ``t'expr={expr}'``:
...
As a result, the ``args`` attribute will always have an odd length, even if the
template string is empty or there are multiple interpolations without static
parts between them:
...
They first thing they need to absorb: unlike f-strings, template string literals
...
TODO should we summarize more of the Discourse and/or more of the early approaches
in this PEP (or even PEP 501)?

Enable Exact Round-Tripping of ``conv`` and ``format_spec``
-----------------------------------------------------------

TODO this feels out of date; can we delete it?
