...
rests entirely with the person choosing to enable lazy imports, not with the
library author. Python libraries are under no obligation to support lazy import
semantics. Politely reporting an incompatibility may be useful to the library
author, but they may choose to simply say their library does not support use
with lazy imports, and this is a valid choice.
...
* Always import needed submodules explicitly, don't rely on some other import
  ...
  explicit ``import bar`` in ``foo/__init__.py``, always do ``import foo.bar;
  foo.bar.Baz``, not ``import foo; foo.bar.Baz``. The latter only works
